...
const I18N = {
  'en-hk': {
    ...
    'about': '<b>Hong Kong Sandbox · 香港沙盒</b>'
      ...
      + '<p>Buildings: Lands Department “Building” layer — 342 000 blocks with surveyed base &amp; roof heights — via the <a href="https://portal.csdi.gov.hk/csdi-webpage/dataset/landsd_rcd_1637211194312_35158" target="_blank" rel="noopener">CSDI portal</a>; land use per block (which windows keep office, shop or home hours) from the Planning Department’s <a href="https://portal.csdi.gov.hk/csdi-webpage/dataset/pland_rcd_1696577406166_85973" target="_blank" rel="noopener">2023 Raster Grids on Land Utilization</a> (both DATA.GOV.HK Terms of Use).</p>'
  ...
  'zh-hk': {
    ...
    'about': '<b>香港沙盒 · Hong Kong Sandbox</b>'
      ...
      + '<p>建築物：地政總署「建築物」圖層 — 342 000 幢樓宇座體，附實測基座及天台高程 — 經 <a href="https://portal.csdi.gov.hk/csdi-webpage/dataset/landsd_rcd_1637211194312_35158" target="_blank" rel="noopener">空間數據共享平台（CSDI）</a> 取得；各座體的土地用途（決定窗戶跟隨辦公、商舖或住宅作息）來自規劃署 <a href="https://portal.csdi.gov.hk/csdi-webpage/dataset/pland_rcd_1696577406166_85973" target="_blank" rel="noopener">2023 年土地利用柵格數據</a>（均為 DATA.GOV.HK 使用條款）。</p>'
...
function renderSky() {
  ...
  // HKS-114: the city's windows light up as the real sky darkens, and how many stay lit
  // follows the sky sim's Hong Kong clock — most in the evening, a scattered few by 4 am
  // (never none), the early risers before dawn. The dusk signal runs on the sun's altitude
  // (0 at +3°, 1 at −12° — sunset to nautical dusk, ~an hour in Hong Kong) so the switch-on
  // is gradual and buildings.js can stagger it by use (shops first, homes last); the sky
  // luminance path stays as a floor so an eclipse or a stargazing night still lights the city.
  { const d = simDate();
    const dusk = Math.max(1 - S01((skyLum - 0.004) / 0.086), cel ? S01((3 - cel.sunAlt / D2R) / 15) : 0);
    bld.setNight(dusk, d ? ((d.getTime() / 3.6e6 + 8) % 24) : null); }
