...
const base='city/data/whampoa-light-trial/',manifest=await fetch(base+'manifest.json').then(r=>r.json()),data=await fetch(base+'basic.json').then(r=>r.json()),loader=new GLTFLoader(),views=[],nf=n=>n.toLocaleString('en-HK'),kb=n=>(n/1024).toFixed(1)+' KiB';let syncing=false,selected='ship',spinning=false,lastFrame=0;
...
function show(id){
 ...
 for(const entry of entries)entry.models.forEach((model,i)=>{const v=views[i];if(model)v.model.add(model);else{const el=document.createElement('div');el.className='placeholder';el.textContent='High-detail pass pending';v.host.append(el);}});
 ...
 window.__trial={ready:true,id,spinning,counts,uids:entries.flatMap(e=>e.group.uids),highUids:references.flatMap(r=>r.high.map(m=>m.uid)),highPending:!highForms,locations:entries.map(e=>e.group.id)};reset();
}
