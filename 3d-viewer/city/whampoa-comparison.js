...
const base='city/data/whampoa-light-trial/',manifest=await fetch(base+'manifest.json').then(r=>r.json()),data=await fetch(base+'basic.json').then(r=>r.json()),loader=new GLTFLoader(),views=[],nf=n=>n.toLocaleString('en-HK'),kb=n=>(n/1024).toFixed(1)+' KiB';let syncing=false,selected='ship';
...
for(const id of ['basic','light','high']){const host=document.getElementById(id),scene=new T.Scene();scene.background=new T.Color(0xe8eee1);const camera=new T.PerspectiveCamera(36,1,.1,4000),renderer=new T.WebGLRenderer({antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1;renderer.shadowMap.enabled=true;host.append(renderer.domElement);const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=false;controls.minDistance=20;controls.maxDistance=1400;controls.maxPolarAngle=Math.PI*.48;scene.add(new T.HemisphereLight(0xffffff,0x596b50,2));const sun=new T.DirectionalLight(0xfff5de,3);sun.position.set(-120,210,110);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-300,right:300,top:300,bottom:-300,near:1,far:700});sun.shadow.bias=-.001;scene.add(sun);const ground=new T.Mesh(new T.PlaneGeometry(1500,1500),new T.MeshStandardMaterial({color:0xdce5d3,roughness:1}));ground.rotation.x=-Math.PI/2;ground.receiveShadow=true;scene.add(ground);const v={id,host,scene,camera,renderer,controls,ground,model:null};views.push(v);controls.addEventListener('change',()=>{if(syncing)return;syncing=true;for(const w of views)if(w!==v){w.camera.position.copy(camera.position);w.controls.target.copy(controls.target);w.controls.update();}syncing=false;render();});new ResizeObserver(()=>{const r=host.getBoundingClientRect();renderer.setSize(r.width,r.height,false);camera.aspect=r.width/r.height;camera.updateProjectionMatrix();render();}).observe(host);}
function render(){for(const v of views)v.renderer.render(v.scene,v.camera);}
