...
function sectionsFor(record,width=record.width,lift=0){
 ...
 // Coincident source nodes are retained when their supplied heights differ.
 // Borrow the adjacent direction only for the cross-section, never its height.
 for(let i=0;i<directions.length;i++)if(!directions[i])directions[i]=directions[i-1]||first;
...
export function geometryFor(record,featureIndex=0){return solidGeometry(sectionsFor(record),DECK_THICKNESS,featureIndex);}
...
function mergeGeometries(geometries){
 const geometry=new THREE.BufferGeometry();
 for(const name of ['position','normal','bridgeFeature']){
...
export class BridgeLayer {
 ...
 async load(url){
  ...
  const promise=Promise.resolve().then(async()=>{
   try{
    ...
    const prepared=await this.prepare(data,this.sampler);
    if(this.closed||controller.signal.aborted)return false;
    ...
    for(const record of prepared){
     ...
     const [minX,minZ,maxX,maxZ]=record.bounds,pad=record.width;
 ...
 buildTile(key){
  ...
  group.name='Footbridge tile '+key;group.userData.bridgeCount=tile.records.length;group.userData.deckMeshes=[];
  for(const record of tile.records){
   const featureIndex=this.featureIndices.get(record.id),style=record.kind==='steps'?'steps':'deck';
 ...
 get stats(){return {spans:this.records.size,visibleSpans:this.group.visible?[...this.cache.values()].filter(group=>group.visible).reduce((count,group)=>count+group.userData.bridgeCount,0):0,tiles:this.cache.size,pending:this.requests.size,errors:[...this.errors.keys()],covered:this.coveredCount,estimatedElevation:this.estimatedCount};}
 dispose(){
  ...
  this.group.removeFromParent();this.records.clear();this.loadedIds.clear();this.tiles.clear();this.features.length=0;this.featureIndices.clear();this.loadedPackages.clear();this.errors.clear();this.coveredCount=this.estimatedCount=0;
 }
}
