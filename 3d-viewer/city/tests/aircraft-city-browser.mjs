...
const showAircraft=async p=>{if(!await p.locator('#flight-controls').isVisible())await p.locator('#fly-toggle').click();};
const chooseAircraft=async(p,id)=>{await showAircraft(p);await p.locator(`[data-aircraft="${id}"]`).click();};
const fly=async p=>{await showAircraft(p);await p.locator('[role=menuitemradio][aria-checked=true]').click();await p.waitForFunction(()=>window.__city.state.mode==='fly'&&window.__city.state.movementReady,null,{timeout:120000});await p.waitForTimeout(1200);};
...
try{
 await ready(page);await fly(page);assert.equal(await page.locator('#aircraft-model [data-aircraft]').count(),7);
 for(const definition of AIRCRAFT){
  await chooseAircraft(page,definition.id);await loaded(page,definition.id);await page.waitForTimeout(550);
  const s=await state(page),status=await page.locator('#aircraft-status').textContent();
  assert.equal(s.mode,'fly');assert.equal(await page.locator('#aircraft-model [aria-checked=true] b').textContent(),definition.label);assert.equal(s.aircraft.dimensions.length,definition.length);assert.equal(s.aircraft.dimensions.wingspan,definition.wingspan);
 ...
 await chooseAircraft(page,'cx747');await entered;await chooseAircraft(page,'prop');await loaded(page,'prop');const lateResponse=page.waitForResponse(response=>response.url().endsWith('/plane-747.glb'));release();await(await lateResponse).finished();await page.waitForTimeout(600);
 assert.equal((await state(page)).aircraft.id,'prop');assert.equal(await page.locator('[data-aircraft=prop]').getAttribute('aria-checked'),'true');await page.unroute('**/plane-747.glb');checks.push('a late model response cannot replace the latest aircraft selection');
 await page.locator('#panel-toggle').click();await page.locator('#tab-time').click();await page.locator('#time').fill('00:00');await page.emulateMedia({reducedMotion:'reduce'});await page.waitForFunction(()=>window.__city.state.lighting.shimmer===0&&window.__city.state.lighting.night>.99);await page.locator('#panel-toggle').click();await page.waitForTimeout(250);await screenshot(page,'city-midnight-reduced-motion-1440x1000.png');checks.push('midnight flight and reduced-motion settings integrate without graphics errors');
 ...
 await retryPage.route('**/plane-a350.glb',r=>r.abort('failed'));await ready(retryPage);await fly(retryPage);await chooseAircraft(retryPage,'a350');
 await retryPage.waitForFunction(()=>window.__city.state.aircraft.id==='a350'&&window.__city.state.aircraft.status==='fallback');await showAircraft(retryPage);assert.equal(await retryPage.locator('#aircraft-retry').isVisible(),true);assert.match(await retryPage.locator('#aircraft-status').textContent(),/Detailed aircraft unavailable/);assert.equal((await state(retryPage)).mode,'fly');await retryPage.waitForTimeout(1200);await screenshot(retryPage,'city-model-fallback-1440x1000.png');
 ...
 for(const width of [390,320]){
  await page.setViewportSize({width,height:844});await chooseAircraft(page,'cx777');await loaded(page,'cx777');await page.waitForTimeout(350);
  await showAircraft(page);const flight=await visibleRect(page,'#flight-controls');assert.ok(flight&&flight.x>=0&&flight.x+flight.width<=width);
