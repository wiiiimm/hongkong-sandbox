...
try{
 ...
 await page.locator('#light-shimmer').uncheck();assert.equal((await state()).lighting.shimmer,0);await page.locator('#light-shimmer').check();assert.equal((await state()).lighting.shimmer,1);
 ...
 await page.setViewportSize({width:390,height:844});await page.locator('#panel-toggle').click();await page.touchscreen.tap(...await point(6));assert.equal((await state()).time,6);await page.touchscreen.tap(...await point(18));assert.equal((await state()).time,18);
