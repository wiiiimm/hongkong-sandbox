...
const time=async value=>{await tab('sky');await page.locator('#time').fill(value);await page.waitForFunction(h=>window.__city.state.time===h,Number(value.slice(0,2))+Number(value.slice(3))/60);await tab('places');await page.waitForTimeout(250);};
