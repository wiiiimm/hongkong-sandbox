...
export function createComparisonGallery(groups, assemble) {
 ...
 function fit(row){
  ...
  if(motion==='fly'){
   // A continuous 32-second approach, facade rise, rooftop pass and wide reveal.
   // Phase offsets give each location its own flight; all variants share the camera.
   const keys=[[-1.15,.32,1.5,.35],[-.3,.18,.88,.3],[.65,1.5,.92,.7],[2.1,1.15,1.05,.55],[4.25,.5,1.6,.4],[Math.PI*2-1.15,.32,1.5,.35]];
  ...
  for(let i=0;i<12&&safe.containsPoint(position);i++){distance*=1.2;position=target.clone().addScaledVector(direction,distance);}
  row.shot=shot;
