...
export function prepareInfrastructure(data){
 if(data?.schemaVersion!==1||data.kind!=='tai-o-official-infrastructure'||data.crs!=='EPSG:2326'||data.verticalDatum!=='Hong Kong Principal Datum'||!Array.isArray(data.models))throw new Error('Invalid source infrastructure inventory');
 const seen=new Set();const models=data.models.map(input=>{
  ...
  if(!Array.isArray(input.suppresses)||input.suppresses.some(id=>typeof id!=='string'))throw new Error('Invalid infrastructure replacements '+input.id);
  seen.add(input.id);
