import {bindBuildingProgress} from './building-progress.js?v=20260913-progress4';
...
async function chooseMode(mode){
 aircraftPicker?.close({restoreFocus:false});
...
function onMode(mode){
 ...
 aircraftPicker?.close({restoreFocus:false});syncAircraftUI();
...
function selectBridge(bridge){
 ...
 if(bridge.modelGeometry){
  ...
  const bottom=floorHeights?Math.min(...floorHeights):bounds[0][1],top=floorHeights?Math.max(...floorHeights):bounds[1][1];
  $('building-height').textContent=`${bottom.toFixed(1)}–${top.toFixed(1)} m`;$('building-levels').textContent=bridge.modelGeometry.triangles.toLocaleString('en-HK');$('building-height-label').textContent=deck?'DECK · HKPD':'MODEL · HKPD';$('building-levels-label').textContent='SOURCE TRIANGLES';
  $('building-source').textContent=`Lands Department · ${bridge.sheet} · ${bridge.sourceRevision.slice(0,10)} UTC. Original infrastructure geometry and elevation, including source supports. ${bridge.walkSurface?.elevationBasis||bridge.elevationBasis||''}`;
  $('building-activity').textContent=bridge.walkable?'Public deck surfaces enabled for walking. '+(bridge.publicAccess||bridge.accessNote||''):bridge.publicAccess||bridge.accessNote||'Source geometry retained; public walking access has not been verified.';$('building-osm').href=bridge.publicAccessSource||bridge.sourceUrl||bridge.source;$('building-card').hidden=false;return;
 }
...
function syncAircraftUI(){
 const state=nav.aircraftState,stamp=[state.id,state.status,state.error].join('|');if(stamp===aircraftUIStamp)return;aircraftUIStamp=stamp;
 const definition=AIRCRAFT.find(a=>a.id===state.id)||AIRCRAFT[0];
 aircraftPicker?.sync(state);
...
function bindUI(){
 ...
 $('aircraft-retry').addEventListener('click',()=>selectAircraft(nav.aircraftState.id));syncAircraftUI();
 ...
 document.querySelectorAll('[data-mode]:not([data-mode="fly"])').forEach(b=>b.addEventListener('click',()=>chooseMode(b.dataset.mode)));
...
async function loadBackgroundData(){
 const urls=manifest.officialModelCatalogues||[];backgroundCataloguesRemaining=urls.length;updateStreamStatus();
