...
// ---- HKS-110 fleet rule: landing gear -------------------------------------
// NO CXGear split for this airframe: the source model ships with NO extended
// landing gear at all — its lowest geometry is the engine cowls (world y
// -4.47 vs fuselage belly -2.3; verified by connected-component analysis and
// a software side-render). An earlier heuristic split (components below
// y -2.7 near the centreline) only captured ~1 000 tris of belly-fairing /
// antenna fragments, which then vanished in flight and left the parked plane
// standing on its engines. Instead, loadPlaneModel() lifts this airframe to
// a gear stance and adds simple procedural gear (cfg.gearProc — see main.js).
