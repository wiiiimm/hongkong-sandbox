...
def main():
 ...
 originalInstalled={p['uid'] for p in d['preflight']['parts'] if p['sourceProgress']=='installed'};assert len(originalInstalled)==133
 ...
 for lid,reg in sorted(registry.items()):
  p=pre[lid];members=sorted(set(p['members'])|{r['uid'] for r in rows if lid in r['landmarkIds']});effective=[uid for uid in members if uid not in centre];known=[byuid[uid]for uid in effective if uid in byuid];missing=[uid for uid in effective if uid not in byuid]
