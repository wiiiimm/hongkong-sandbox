...
def complete_state(work,sheet):
 path=work/'sources'/sheet/'state.json'
 if not path.exists():raise ValueError('Missing planned source sheet: '+sheet)
 state=a.read(path)
 if state.get('status')not in ('staged','complete-directory-no-exact-model'):raise ValueError('Incomplete planned source sheet: '+sheet)
 return state

def validate_memberships(plan):
 targets={row['uid']:row for row in plan['targets']}
 assert len(targets)==len(plan['targets'])==plan['targetParts'],'Duplicate or missing target membership'
 for uid,row in targets.items():
  assert row.get('sheets'),'Target has no checked source sheets: '+uid
  for sheet in row['sheets']:assert sheet in plan['tiles']and uid in plan['tiles'][sheet]['uids'],'Target/sheet membership mismatch: '+uid
 for sheet,tile in plan['tiles'].items():
  for uid in tile['uids']:assert uid in targets and sheet in targets[uid]['sheets'],'Sheet/target membership mismatch: '+sheet
...
def main():
 ...
 try:validate_memberships(plan)
 except Exception as error:errors.append({'plan':str(error)})
 for sheet in plan['tiles']:
  folder=work/'sources'/sheet
  try:
   state=complete_state(work,sheet)
