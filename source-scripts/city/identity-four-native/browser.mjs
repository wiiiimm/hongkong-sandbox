...
const path='source-scripts/city/identity-four-native/review-candidates/',catalogue=await read(path+'catalogue.json'),models=new Map(catalogue.models.map(m=>[m.uid,m]));
const groups=[{id:'cullinan-elements',uids:['landsd/203728:0','landsd/203724:0','landsd/273061:0']},{id:'china-resources',uids:['landsd/37369:0']},{id:'oakhill',uids:['landsd/323059:0']}];
...
const output=new URL('docs/astra-city/identity-four-native/browser/',root);await mkdir(output,{recursive:true});
...
try{
 for(const mobile of process.env.IDENTITY_FOUNDATIONS_ONLY? [false]:[false,true]){
  ...
  let inject=false;const url='city/data/official-models/identity-four-native/catalogue.json';
