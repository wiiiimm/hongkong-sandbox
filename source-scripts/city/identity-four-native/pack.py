...
ROOT=pathlib.Path(__file__).resolve().parents[3];HERE=pathlib.Path(__file__).resolve().parent;DOC=ROOT/'docs/astra-city/identity-four-native';DOC.mkdir(parents=True,exist_ok=True)
sys.path.insert(0,str(ROOT/'source-scripts/city/shared-modelling'));import reservations
for p in ['/tmp/astra-identity-four-review-lease.json']:assert reservations.owns(json.loads(pathlib.Path(p).read_text()))
...
BATCH=ROOT/'source-scripts/city/landmark-acquisition/batches/identity-four-native-20260909'
plan=read(BATCH/'plan.json');results=[];held=[]
for target in plan['targets']:
 b=dict(c.execute('SELECT * FROM buildings WHERE uid=? AND active=1',(target['uid'],)).fetchone());options=[]
 for path in (BATCH/'staged').glob('*/manifest.json'):
  m=read(path)
  for s in m['models']:
 ...
 if len(options)!=1:
  held.append({'uid':target['uid'],'outcome':'exact-georef-model-acquired-footprint-match-held','sourceMatches':len(options),'sourceBatch':str(BATCH.relative_to(ROOT))});continue
...
c.close();catalogue=read(ROOT/'source-scripts/city/grounded-model-review/candidates/catalogue.json');catalogue.update(area='Four alias-resolved government towers; placement pending',models=[r['record']for r in results],counts={'packedModels':len(results)})
(out/'catalogue.json').write_text(json.dumps(catalogue,indent=2)+'\n');(DOC/'packing.json').write_text(json.dumps({'issue':'HKS-214','results':results,'held':held,'published':False,'sourceBytesPreserved':True},indent=2)+'\n')
