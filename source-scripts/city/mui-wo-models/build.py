...
def build_models(source_dir=HERE, official_path=None, buildings=None, tiles=TILES, doc=DOC, existing_assets=None, preserved_models=None, scope=None):
 official=official_path or ROOT/'source-scripts/city/mui-wo-buildings/landsd-mui-wo.json.gz'
 by_csuid=collections.defaultdict(list)
 for b in (buildings if buildings is not None else json.loads((ROOT/'3d-viewer/city/data/mui-wo-buildings.json').read_text())['buildings']):by_csuid[b['buildingCSUID']].append(b['uid'])
 manifests=[];summaries=[]
 existing=REVIEW/'model-sample'
 for assets in ([existing] if existing_assets is None else existing_assets):
  manifest=json.loads((assets/'manifest.json').read_text());manifests.append((assets,manifest))
 for tile in tiles:
  source=source_dir/'sources'/tile;assets=source_dir/'staged'/tile;download=json.loads((source/'download.json').read_text())
 ...
 for assets,manifest in manifests:
  for spec in manifest['models']:
   ...
   reason=None
   if not csuids:reason='No exact GeoRefNo plus footprint/centroid match in the retained official selection'
 ...
 original=json.loads((existing/'model-geometries.json').read_text())['byBuildingUid'] if preserved_models is None else preserved_models
 ...
 payload={'schemaVersion':1,'kind':'official-building-model-geometries','datasetId':manifest['datasetId'],'crs':'EPSG:2326','origin':[834500,816500],'verticalDatum':'Hong Kong Principal Datum','coordinatePolicy':'Exact original glTF node transforms; city translation [-834500,0,816500]. Source positions/normals/colours retained. No height adjustment or simplification.','matchPolicy':manifest['matchPolicy'],'scope':scope or 'Only verified single-component BuildingCSUID matches in the retained user-supplied Mui Wo bbox are replaced. Unmatched/outside/ambiguous models are accounted for but not added as duplicate buildings.','sourceManifests':[{'file':str((a/'manifest.json').relative_to(ROOT)),'sha256':hashlib.sha256((a/'manifest.json').read_bytes()).hexdigest(),'tile':m['tile'],'revision':m['tileRevision'],'counts':m['counts']} for a,m in manifests],'counts':counts,'byBuildingUid':geometries}
 path=source_dir/'model-geometries.json.gz';digest=dump(path,payload)
