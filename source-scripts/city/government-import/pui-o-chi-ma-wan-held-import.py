"""Publish the resolved Pui O / Chi Ma Wan holds and persist terminal states."""
...
GOOD_TO_GO = {"landsd/195308:0", "landsd/196549:0"}
...
def verify_browser(path, uids, shoreline=False):
    report = read(path)
    ...
    for row in report["views"]:
        ...
        if row["uid"] == SHORELINE_UID and shoreline:
            if row.get("ground") is not None:
                assert abs(row["ground"] - row["groundSampler"]) <= 5.3
        else:
            assert abs(row["ground"] - row["groundSampler"]) <= 0.004
    return report
...
def start():
    ...
    assert len(all_uids) == 64 and set(all_uids) >= GOOD_TO_GO
    claim = reservations.claim(
        "codex-pui-o-chi-ma-wan-held-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert result["models"] == 62 and set(result["goodToGo"]) == GOOD_TO_GO
    ...
    shoreline = read(DOC / "shoreline-support-proof.json")
    assert shoreline["accepted"] and shoreline["uid"] == SHORELINE_UID
    assert shoreline["retainsBasicForm"] and shoreline["aiCalls"] == shoreline["modelGeometryChanges"] == 0
    ...
    assert len(installed) == 62 and len(all_uids) == 64 and not (installed & GOOD_TO_GO)
    ...
    assert len(metrics["rows"]) == 62 and metrics["aiCalls"] == metrics["geometryChanges"] == 0
    ...
    for uid, metric in metric_by_uid.items():
        ...
        if uid != SHORELINE_UID:
            assert metric["missingTerrain"] == 0 and metric["maxSamplerDelta"] <= 0.004

    validation = read(DOC / "validation.json")
    exceptions = [row for row in validation["results"] if row["outcome"] == "validation-exception"]
    assert validation["models"] == validation["loaderAccepted"] == 62
    assert exceptions == [{
        "uid": SHORELINE_UID,
        "outcome": "validation-exception",
        "loaderAccepted": True,
        "error": "Expected a rendered terrain surface",
    }]
    ...
    assert browser_config["allowMissingRenderedGroundUids"] == [SHORELINE_UID]
    assert browser_config["samplerToleranceByModel"] == {SHORELINE_UID: 5.3}
    verify_browser(DOC / "staged-browser.json", installed, shoreline=True)
    ...
    evidence_paths = {
        ...
        "shorelineSupportProofSHA256": DOC / "shoreline-support-proof.json",
    ...
    decision = {
        "policy": "original-government-pui-o-chi-ma-wan-held-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            "The final 64 Pui O / Chi Ma Wan source forms reached terminal compute-only states: 62 unchanged "
            "government sources installed and 2 current forms accepted as good to go because their government "
            "sources add no visible detail. No AI review or geometry edits."
        ),
    })
    ...
    try:
        ...
        verify_browser(DOC / "live-browser.json", installed, shoreline=True)
    ...
    # Record the two current-form acceptances after publication so their input hashes include the final manifest.
    ...
    assert progress["breakdown"]["goodToGo"] >= 2
    assert progress["government"]["goodToGo"] >= 2
    ...
    stage_name = "government-pui-o-chi-ma-wan-held-terminal-v1"
