"""Publish the compute-only Tai O / northwest villages held-model second pass."""
...
ROOT = HERE.parents[2]
SOURCE_BATCH = "government-tai-o-northwest-600-20260921"
BATCH = "government-tai-o-northwest-held-78-20260921"
SOURCE_DOC = ROOT / "docs/astra-city/government-import" / SOURCE_BATCH
DOC = SOURCE_DOC / "held-78"
STAGE = HERE / "accepted" / BATCH
LOCAL = HERE / "local" / BATCH
RUNTIME_HOLD_UID = "landsd/112959:0"
...
def start():
    ...
    assert len(all_uids) == 78 and RUNTIME_HOLD_UID in all_uids
    claim = reservations.claim(
        "codex-tai-o-northwest-held-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert result["models"] == 77 and result["held"] == [RUNTIME_HOLD_UID]
    ...
    held = {RUNTIME_HOLD_UID}
    all_uids = installed | held
    assert len(installed) == 77 and len(all_uids) == 78 and not (installed & held)
    ...
    assert len(metrics["rows"]) == 77 and metrics["aiCalls"] == metrics["geometryChanges"] == 0
    ...
    assert validation["models"] == validation["loaderAccepted"] == validation["checksPassed"] == 77
    ...
    decision = {
        "policy": "original-government-tai-o-northwest-held-compute-only-v1",
        "sourceBatch": SOURCE_BATCH,
        "publicationBatch": BATCH,
        "modelsProcessed": 78,
        "installedUids": sorted(installed),
        "heldRows": [{
            "uid": RUNTIME_HOLD_UID,
            "reasons": source_terminal[RUNTIME_HOLD_UID]["reasons"],
            "validationError": "Official model does not fit its matched footprint",
            "requiresAI": False,
            "requiresUserDecision": False,
            "needsMoreCompute": False,
            "scriptedWorkComplete": True,
            "nextDependency": "corrected-government-source-or-footprint-identity",
        }],
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            "The Tai O / northwest villages second compute pass installed 77 unchanged government sources after "
            "stitched-terrain, retained-support, runtime and representative browser checks. One invalid source-to-footprint "
            "match remains held. No AI review or geometry edits."
        ),
    })
    ...
    held_observation = (
        "Government source fails the deterministic source-to-footprint runtime identity check; current basic form retained. "
        "No additional compute, AI review or user decision can safely install this unchanged source."
    )
    ledger.record_many(
        snapshot,
        receipt_path,
    ...
    ledger.record_many(
        snapshot,
        receipt_path,
        [(RUNTIME_HOLD_UID, "held", DOC / "decision.json", held_observation, commit)],
        effort=effort,
    ...
    terminal_result = {
        ...
        "heldReasonCounts": {"below-grade-source-surfaces-exceed-bounded-foundation-policy": 1},
        "installedUids": sorted(installed),
    ...
    stage_name = "government-tai-o-northwest-held-terminal-v1"
