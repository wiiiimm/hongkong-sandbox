"""Process all matched, uninstalled Discovery Bay / Nim Shue Wan government forms; never AI."""
...
INVENTORY = DOC / "pre-batch-inventory/buildings.csv"
...
def prepare():
    ...
    frozen = {
        ...
        "selectionPolicy": "All matched uninstalled forms in review section 10.5 Discovery Bay and Nim Shue Wan",
...
def recover():
    """Recover the frozen batch in bounded chunks, then restore one audited result."""
    frozen = xl.read(LOCAL / "selection.json.gz")
    assert xl.h(ROOT / "3d-viewer/city/data/manifest.json") == frozen["manifestSHA256"]
    evidence = xl.read(LOCAL / "recovery-inputs.json.gz")
    assert len(evidence["rows"]) == 1103
    native_by_uid = {
        match["uid"]: outcome
        for outcome in evidence["native"]
        for match in outcome["model"]["matching"]["viewerMatches"]
        if match["uid"] in evidence["rows"]
    }
    assert set(native_by_uid) == set(evidence["rows"])
    combined_rows = []
    combined_errors = {}
    combined_methods = collections.Counter()
    part_summaries = []
    for number, start in enumerate(range(0, len(evidence["rows"]), 1000), 1):
        uids = evidence["rows"][start:start + 1000]
        part = {
            "nativeRun": evidence["nativeRun"],
            "rows": uids,
            "sources": {uid: evidence["sources"][uid] for uid in uids},
            "native": [native_by_uid[uid] for uid in uids],
        }
        part_path = LOCAL / f"recovery-inputs-part-{number}.json.gz"
        xl.save(part_path, part)
        result = xl.shape_prepare.prepare(
            part_path, LOCAL / "recovered", allow_source=True, workers=1,
            env_file=ROOT / ".env.modelling",
        )
        combined_rows.extend(result["rows"])
        combined_errors.update(result["errors"])
        combined_methods.update(result["methods"])
        part_summaries.append({
            "part": number, "requested": len(uids), "recovered": len(result["rows"]),
            "errors": len(result["errors"]), "methods": result["methods"],
            "seconds": result["seconds"],
        })
    assert len(combined_rows) + len(combined_errors) == 1103
    result = {
        "rows": sorted(combined_rows, key=lambda row: row["uid"]),
        "errors": combined_errors,
        "methods": dict(combined_methods),
    ...
    xl.save(LOCAL / "recovered/geometry-inputs.json", result)
    xl.save(DOC / "recovery-summary.json", {k: v for k, v in result.items() if k != "rows"})
    print(json.dumps({
        "requested": 1103, "recovered": len(combined_rows),
        "errors": len(combined_errors), "methods": dict(combined_methods),
        "parts": part_summaries, "aiCalls": 0,
    }), flush=True)
    return result
...
def reserve_and_check():
    ...
    receipts = []
    try:
        for start in range(0, len(frozen["rows"]), 1000):
            chunk = frozen["rows"][start:start + 1000]
            receipt = xl.reservations.claim(
                "codex-discovery-bay-" + str(uuid.uuid4()),
                ["building:" + row["uid"] for row in chunk], batch=BATCH, ttl=3600,
            )
            assert receipt["ok"], "Existing source owner; no takeover"
            receipts.append(json.loads(json.dumps(receipt["reservation"], default=str)))
        xl.save(LOCAL / "reservation.json", receipts)
        xl.save(LOCAL / "reservation-first.json", receipts[0])
        xl.save(LOCAL / "reservation-second.json", receipts[1])
        xl.command([
            sys.executable, str(HERE.parent / "shared-modelling/reservations.py"), "run",
            "--lease-file", str(LOCAL / "reservation-first.json"), "--",
            sys.executable, str(HERE.parent / "shared-modelling/reservations.py"), "run",
            "--lease-file", str(LOCAL / "reservation-second.json"), "--",
            sys.executable, __file__, "check-owned",
        ])
    finally:
        for receipt in receipts:
            xl.reservations.release(receipt)
