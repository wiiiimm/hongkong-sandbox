"""Stage unchanged Discovery Bay / Nim Shue Wan sources with bounded native terrain."""
...
SOURCE='government-discovery-bay-1103-20260921';BATCH='government-discovery-bay-compute-20260921'
...
def partition(groups, obstacles):
 """Partition candidate coverage into disjoint rectangles around installed patches."""
 output=[]
 for parent_url in sorted({group['parentURL'] for group in groups}):
  source=[group for group in groups if group['parentURL']==parent_url]
  occupied=set()
  for group in source:
   x0,z0,x1,z1=group['cells']
   occupied.update((x,z) for z in range(z0,z1) for x in range(x0,x1))
  blocked=set()
  for x0,z0,x1,z1 in obstacles.get(parent_url,()):
   blocked.update((x,z) for z in range(z0,z1) for x in range(x0,x1))
  assert not occupied&blocked,'candidate-terrain-overlaps-installed-patch'
  while occupied:
   x0,z0=min(occupied,key=lambda cell:(cell[1],cell[0]))
   x1=x0+1
   while (x1,z0) in occupied:x1+=1
   z1=z0+1
   while all((x,z1) in occupied for x in range(x0,x1)):z1+=1
   cells=[x0,z0,x1,z1]
   uids=sorted({uid for group in source if terrain_helpers.overlap(cells,group['cells']) for uid in group['uids']})
   assert uids
   output.append({'uids':uids,'cells':cells,'parentURL':parent_url})
   occupied.difference_update((x,z) for z in range(z0,z1) for x in range(x0,x1))
 return output
...
def sampled_patch(group,parent,triangles,sources,parent_url,parent_sha,patch_id):
 ...
 estimated=(round((bounds[2]-bounds[0])/step)+1)*(round((bounds[3]-bounds[1])/step)+1)
 if estimated>100000:step=5
...
def main():
 ...
 regional={'city/data/terrain-mui-wo.json','city/data/terrain-tai-o.json','city/data/terrain-pui-o.json','city/data/terrain-ngong-ping.json'};installed_top=[]
 ...
 obstacles={url:[child['coarseCells'] for child in parent['data'].get('patches',[])] for url,parent in parents.items()}
 obstacles['city/data/terrain.json'] += [x['data']['coarseCells'] for x in installed_top]
 groups=partition(groups,obstacles);proofs={r['sheet']:r for r in read(DOC/'source-terrain.json')['sheets']};terrains={};terrain_bounds={}
 ...
 shutil.rmtree(STAGE/'terrain',ignore_errors=True);STAGE.mkdir(parents=True,exist_ok=True);nested={url:[] for url in regional};top=[];patch_rows=[];accepted=set()
 for number,group in enumerate(groups,1):
  ...
  try:
   ...
   patch_id=f'government-discovery-bay-{number:03d}'
   try:
    fine_step=min(5,abs(parent['data']['meta']['georef']['aE'])/5)
    fine_points=(round((bounds[2]-bounds[0])/fine_step)+1)*(round((bounds[3]-bounds[1])/fine_step)+1)
    if fine_points>100000 or len(group['uids'])>50:raise ValueError('region-scale-group-sampled-fallback')
   ...
   patch['id']=patch_id
   if parent['top']:
    path=STAGE/'terrain'/(patch['id']+'.json');save(path,patch);top.append({'source':rel(path),'sha256':sha(path),'destination':'city/data/'+patch['id']+'.json','resolution':patch['cell'],'area':'Discovery Bay / Nim Shue Wan government buildings'})
 ...
 base=read(HERE.parent/'kai-tak-port/staged/catalogue.json');catalogue={k:v for k,v in base.items() if k not in ('models','counts','area')};catalogue.update(area='Discovery Bay / Nim Shue Wan original government buildings',loadingPolicy='Lazy progressive loading after deterministic source, terrain, runtime and browser checks',counts={'packedModels':len(models)},models=models)
