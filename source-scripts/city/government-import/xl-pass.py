"""Bounded first pass of 50 high-complexity XL sources. Original meshes only; no AI or exception remodelling."""
import argparse
...
BATCH='government-xl-50-20260913'
LIMIT=50
GROUP='xl'
...
def select_entries(inventory, installed, limit=LIMIT):
    entries=[];seen=set()
    ...
    if len(entries)!=limit:raise ValueError('Not enough uniquely matched, uninstalled XL sources')
    return entries
...
def prepare():
    ...
    frozen={'batch':BATCH,'nativeRun':NATIVE_RUN,'rows':rows,'manifestSHA256':h(ROOT/'3d-viewer/city/data/manifest.json'),'selectionPolicy':'highest-triangle uniquely matched uninstalled XL sources','sizeGroup':GROUP,'selectionLimit':LIMIT,'sourceCommit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'aiCalls':0}
