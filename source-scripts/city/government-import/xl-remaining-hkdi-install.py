"""Publish two verified unchanged HKDI XL wings through the guarded reviewer."""
...
DOC = ROOT / "docs/astra-city/government-import/government-xl-remaining-20260923/hkdi"
...
def owned():
    ...
    for model in catalogue["models"]:
        ...
        parts[model["uid"]] = {
            ...
            "classification": "script-verified-original-government-xl-same-parent",
            "knownHold": False,
        }
    ...
    save(inventory_path, {**inventory, "snapshotId": snapshot, "derivedFrom": pointer["snapshotId"],
                          "parts": ordered, "qualification": "Two unchanged government HKDI XL wings accepted by exact identity, bounded same-parent overlap, current terrain, runtime and staged browser checks. No geometry changes or per-model AI."})
    ...
    observation = ("Exact unchanged government HKDI XL source. Object ID and Building CSUID, source hash, "
                   "projected identity, bounded same-parent overlap, current terrain, mobile budget and staged "
                   "desktop/mobile day/night, picking, collision, fallback and retry checks passed. "
                   "No AI model review or geometry edits.")
