"""Exact source-terrain pass for the Discovery Bay / Nim Shue Wan government batch.
...
def fetch_sheet(sheet: str):
    ...
    try:
        ...
        archive_path = out / f"{sheet}.zip"
    except urllib.error.HTTPError as error:
        if error.code != 502:
            raise
        spec = importlib.util.spec_from_file_location("full_source", HERE / "discovery-bay-full-source-recovery.py")
        full_source = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(full_source)
        archive_path = source / "full-source" / f"{sheet}.zip"
        if not archive_path.exists() or archive_path.stat().st_size != row["archiveBytes"]:
            full_source.download(row, archive_path)
        archive_sha = full_source.digest(archive_path)
        record = {
            "revisionDate": full_source.read(RECOVERED / "source-metadata.json")["directories"][sheet]["revision"],
            "sourceETag": row["etag"],
            "directorySHA256": row["directorySHA256"],
            "sha256": archive_sha,
            "bytes": archive_path.stat().st_size,
            "sourceArchiveBytes": row["archiveBytes"],
            "receivedBytes": archive_path.stat().st_size,
            "newThisInvocationBytes": 0,
            "cacheKind": "verified-complete-government-source-archive",
        }
    decoded = out / "decoded"
    members = []
...
def fetch():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                failures.append({"sheet": sheet, "error": f"{type(error).__name__}: {error}"})
                print(json.dumps({
                    "terrainSheets": len(results), "totalSheets": len(sheets),
                    "sheet": sheet, "held": failures[-1]["error"],
                }), flush=True)
                continue
    ...
    failures.sort(key=lambda row: row["sheet"])
    report = {
        "batch": BATCH,
        "sheets": results,
        "sourceSheets": len(results),
        "expectedSourceSheets": len(sheets),
        "failedSheets": failures,
...
def exact():
    ...
    assert set(geometries) <= set(selection) and len(selection) == 1103
    assert all(validation[uid]["outcome"] == "validation-exception" for uid in missing)
    terrain_report = read(DOC / "source-terrain.json")
    terrain_proof = {row["sheet"]: row for row in terrain_report["sheets"]}
    terrain_failures = {row["sheet"]: row["error"] for row in terrain_report.get("failedSheets", [])}
    terrains = {}
    ...
    for number, uid in enumerate(sorted(geometries), 1):
        ...
        source_sheet = row["native"]["sheet"]
        if source_sheet not in terrains:
            results.append({
                "uid": uid,
                "modelId": row["candidate"]["entry"]["modelId"],
                "name": row["candidate"]["entry"].get("label"),
                ...
                "suppressesBuildingUids": [],
                "publicationCandidate": False,
                "reasons": ["source-terrain-download-unavailable"],
                "validationError": terrain_failures.get(source_sheet, "source terrain sheet unavailable"),
                "humanStatus": "held-unknown",
                "requiresAI": False,
                "requiresUserDecision": False,
                "needsMoreCompute": False,
            ...
            continue
        foundation = foundation_context(triangles, terrains[source_sheet])
        identity_clear = (
