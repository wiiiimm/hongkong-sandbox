"""Exhaust deterministic geometry checks for the bounded XL-50 source batch.
...
spec = importlib.util.spec_from_file_location("xl_second", Path(__file__).with_name("xl-second-pass.py"))
...
def remaining_rows():
    ...
    assert len(rows) == 50, f"Expected 50 uninstalled XL sources, found {len(rows)}"
...
def native_terrain():
    ...
    pieces = [s.terrain_triangles(ROOT / path) for source in sources for path in source["terrainPaths"]]
...
def main():
    ...
    report = {
        ...
        "qualification": "All 50 XL sources received exact identifier, projected assembly, vertical neighbour and connected below-grade source-face checks. Results are diagnostic only until normal runtime, neighbour and browser publication gates pass.",
    }
