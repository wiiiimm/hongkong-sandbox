"""Stage four unchanged Beverly Hill XL towers with their existing solid podium."""
...
def owned():
    ...
    for uid in UIDS:
        ...
        assert direct.identity_clear(proof)
        ...
        assert -.1 <= support["candidates"][0]["verticalMarginM"] <= 5
        ...
        entry.update(priority="landmark", placementReviewed=True, sourceIdentityReviewed=True,
                     ...
                     placementReview=f"Exact unchanged government tower supported by unchanged solid basic podium {SUPPORT_UID}. Source low-rim hull and target footprint coverage, source hash, identifiers, terrain surface and mobile runtime passed. No AI or geometry edits.")
    ...
    template.update(area="Remaining XL unchanged government towers with retained podium",
    ...
    decision = {"batch": BATCH, "policy": "original-government-xl-unchanged-podium-v1",
