"""Stage a Pui O/Mui Wo terrain-overlap correction and unchanged substation source."""
import gzip,hashlib,json,shutil,subprocess
from pathlib import Path
HERE=Path(__file__).resolve().parent;ROOT=HERE.parents[2]
BATCH='government-lantau-pui-o-overlap-probe-20260921';STAGE=HERE/'local'/BATCH;DOC=ROOT/'docs/astra-city/government-import'/BATCH
...
def main():
 ...
 prior=list(pui['renderedElev']);corrected=list(prior);left=177;right=182;last=19
 def mui_y(c,r):
 ...
 for r in range(0,last+1):
  for c in range(left,pui['w']):
   ...
   alpha=min(1,max(0,(c-left)/(right-left)))
   new=round(old+(desired-old)*alpha,6)
   if new!=old:changed+=1;max_delta=max(max_delta,abs(new-old))
   corrected[i]=new
 # The common 5m cells are drawn only by the earlier Mui Wo patch. The
 # shared boundary row and a 25m exterior band join Pui O without a cliff.
 pui['renderedElev']=corrected;pui['patchExclusions']=[[dc,0,pui['w']-1,14]]
 pui['meta']['renderedTransition']={**pui['meta'].get('renderedTransition',{}),'regionalOverlapCorrection':{'policy':'Earlier Mui Wo source grid owns common cells; Pui O boundary matches it with a 25m rendered-height transition. Raw DTM elevations unchanged.','muiWoSHA256':sha(A),'puiOOriginalSHA256':sha(B),'excludedCells':pui['patchExclusions'],'changedRenderedVertices':changed,'maxRenderedHeightAdjustmentM':max_delta}}
 terrain=STAGE/'terrain-pui-o-overlap-corrected.json';save(terrain,pui)
 proof={'uid':UID,'policy':'deduplicate-aligned-official-regional-terrain-v1','muiWoSHA256':sha(A),'puiOOriginalSHA256':sha(B),'correctedSHA256':sha(terrain),'rawElevationUnchanged':pui['elev']==read(B)['elev'],'sourceGeometryChanged':False,'excludedCells':pui['patchExclusions'],'changedRenderedVertices':changed,'maxRenderedHeightAdjustmentM':max_delta,'aiCalls':0}
