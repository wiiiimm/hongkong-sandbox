"""Publish one unchanged Discovery Bay source with bounded original terrain."""
...
BATCH = "government-lantau-discovery-native-20260921"
...
def start():
    ...
    assert len(all_uids) == 1 and len(set(all_uids)) == 1
...
def owned():
    ...
    assert len(source_rows) == result["models"] == 1
    held = set()
    ...
    for uid, metric in metric_by_uid.items():
        ...
        assert metric["minSurfaceGap"] >= -1.8
    ...
    terrain = browser_config["terrain"]
    assert len(terrain) == 1 and terrain[0]["destination"] == "city/data/government-native-176056-0.json"
    assert digest(ROOT / terrain[0]["source"]) == terrain[0]["sha256"]
    source_doc = ROOT / "docs/astra-city/government-import/government-lantau-discovery-native-probe-20260921"
    neighbours = read(source_doc / "neighbour-checks.json")
    native = read(source_doc / "native-neighbour-checks.json")
    assert neighbours["patches"][0]["blockedBy"] == [] and native["blocked"] == []
    assert read(DOC / "assembly-proof.json")["source"]["patchSHA256"] == terrain[0]["sha256"]
    ...
    evidence_paths = {
        ...
        "terrainSHA256": ROOT / terrain[0]["source"],
        "neighbourChecksSHA256": source_doc / "neighbour-checks.json",
        "nativeNeighbourChecksSHA256": source_doc / "native-neighbour-checks.json",
    }
    decision = {
        "policy": "original-government-lantau-native-terrain-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"A bounded native source-terrain patch enabled installation of {len(installed)} unchanged government sources after "
    ...
    installed_observation = (
        "Exact unchanged government source and bounded native terrain passed deterministic identity, visible-roof, retained-basic-support, "
        "runtime and desktop/mobile browser checks. No AI review, remodelling, simplification or geometry edit."
    )
    ledger.record_many(
        snapshot,
        receipt_path,
    ...
    terrain_live = ROOT / "3d-viewer" / terrain[0]["destination"]
    assert not terrain_live.exists()
    ...
    try:
        ...
        terrain_live.unlink(missing_ok=True)
    ...
    stage_name = "government-lantau-discovery-native-terminal-v1"
