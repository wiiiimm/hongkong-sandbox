"""Stage Discovery Bay / Nim Shue Wan sources against already installed government terrain; no geometry edits or AI."""
...
ROOT = HERE.parents[2]
SOURCE = "government-discovery-bay-1103-20260921"
...
def main():
    ...
    assert len(models) == terrain["terrainAccepted"] and not (set(models) & existing)
    ...
    for uid in sorted(existing):
        ...
        entry.update(
            ...
            placementReview=(
                "Unchanged Lands Department geometry; exact object/CSUID and source-terrain checks. "
                "Installed terrain retained; runtime/browser contact required. Compute-only import."
            ),
        )
    ...
    print(json.dumps({
        "refinedTerrain": terrain["terrainAccepted"],
