"""Stage unchanged Pui O / Chi Ma Wan sources with bounded native terrain."""
...
def main():
 exact=read(DOC/'exact-pass-results.json.gz');selection={r['uid']:r for r in read(DOC/'check-selection.json.gz')['rows']};candidates={r['uid']:r for r in exact['rows'] if r['publicationCandidate']};assert len(candidates)==314
 ...
 regional={'city/data/terrain-pui-o.json','city/data/terrain-mui-wo.json'};installed_top=[]
 ...
 for number,group in enumerate(groups,1):
  ...
  try:
   ...
   patch_id=f'government-pui-chi-{number:03d}'
   try:
   ...
   patch['id']=patch_id
   if parent['top']:
    path=STAGE/'terrain'/(patch['id']+'.json');save(path,patch);top.append({'source':rel(path),'sha256':sha(path),'destination':'city/data/'+patch['id']+'.json','resolution':patch['cell'],'area':'Pui O / Chi Ma Wan government buildings'})
 ...
 base=read(HERE.parent/'kai-tak-port/staged/catalogue.json');catalogue={k:v for k,v in base.items() if k not in ('models','counts','area')};catalogue.update(area='Pui O / Chi Ma Wan original government buildings',loadingPolicy='Lazy progressive loading after deterministic source, terrain, runtime and browser checks',counts={'packedModels':len(models)},models=models)
