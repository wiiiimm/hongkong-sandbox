"""Freeze the runtime-clean Cheung Sha / Tong Fuk / Shui Hau subset; no AI or geometry edits."""
...
def main():
 ...
 for bundle_name in bundles:
  bundle=read(ROOT/bundle_name);parent=ROOT/'3d-viewer'/bundle['parentTerrainURL'];wrapper={**read(parent),'patches':read(parent).get('patches',[])+bundle['patches']};path=LOCAL/'browser-terrain'/Path(bundle['parentTerrainURL']).name;save(path,wrapper);browser_terrain.append({'source':rel(path),'destination':bundle['parentTerrainURL'],'resolution':wrapper['cell'],'area':'Cheung Sha / Tong Fuk / Shui Hau government terrain','replaces':{'url':bundle['parentTerrainURL']}})
