"""Check exact identity, terrain and runtime on same-member revision recoveries."""
...
BATCH = "government-xl-source-revision-check-20260924"
BASE = ROOT / "docs/astra-city/government-import/government-xl-remaining-20260923"
DOC = BASE / "revision-check-20260924"
...
def start():
    ...
    assert len(uids) == EXPECTED and len(recovered["errors"]) == 2
...
def owned():
    ...
    recovered = read(RECOVERED / "geometry-inputs.json")
    rows = recovered["rows"]
    assert len(rows) == EXPECTED and len(recovered["errors"]) == 2
    ...
    save(DOC / "identity.json", {"rows": [{"uid": uid, "identity": item} for uid, item in identity.items()],
                                  ...
                                  "retrySHA256": sha(BASE / "revision-recovery-20260924.json"), "aiCalls": 0})
