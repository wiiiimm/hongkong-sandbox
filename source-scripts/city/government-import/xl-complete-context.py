"""Fill exact adjacent terrain coverage for the bounded XL-50 batch."""
...
def prepare():
    ...
    claim=reservations.claim('codex-xl-adjacent-'+str(uuid.uuid4()),resources,batch=s.BATCH+'-adjacent');assert claim['ok'];receipt=json.loads(json.dumps(claim['reservation'],default=str));save(LOCAL/'adjacent-reservation.json',receipt)
...
def owned():
    ...
    for source in all_sources:
        ...
        native.extend(s.terrain_triangles(ROOT/p) for p in source['terrainPaths'])
