"""Publish the compute-only Discovery Bay / Nim Shue Wan batch and persist terminal states."""
...
BATCH = "government-discovery-bay-compute-20260921"
...
def verify_browser(path, browser_uids, failure_uids):
    ...
    for row in report["views"]:
        ...
        if read(STAGE / "browser-config.json").get("fitBoxByModel", {}).get(row["uid"], True):
...
def start():
    ...
    assert len(all_uids) == 1103 and len(set(all_uids)) == 1103
    receipts = []
    try:
        for number, chunk in enumerate((all_uids[:1000], all_uids[1000:]), 1):
            claim = reservations.claim(
                "codex-discovery-bay-" + str(uuid.uuid4()),
                ["building:" + uid for uid in chunk], batch=BATCH, ttl=3600,
            )
            assert claim["ok"], claim
            path = LOCAL / f"reservation-{number}.json"
            save(path, json.loads(json.dumps(claim["reservation"], default=str)))
            receipts.append(path)
        call([
            sys.executable, str(HERE.parent / "shared-modelling/reservations.py"),
            "run", "--lease-file", str(receipts[0]), "--",
            sys.executable, str(HERE.parent / "shared-modelling/reservations.py"),
            "run", "--lease-file", str(receipts[1]), "--",
            sys.executable, __file__, "owned",
        ])
    finally:
        for path in receipts:
            reservations.release(read(path))



def owned():
    receipt_paths = [LOCAL / "reservation-1.json", LOCAL / "reservation-2.json"]
    assert all(reservations.owns(read(path)) for path in receipt_paths)
    ...
    assert len(all_uids) == 1103 and len(installed) == installed_count and len(held) == held_count
    assert installed | held == all_uids and not (installed & held)
    assert terminal["modelsProcessed"] == 1103
    assert installed_count + held_count == 1103
    ...
    decision = {
        "policy": "original-government-discovery-bay-compute-only-v1",
        "sourceBatch": SOURCE_BATCH,
        "publicationBatch": BATCH,
        "modelsProcessed": 1103,
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"{installed_count} unchanged Discovery Bay / Nim Shue Wan government sources installed after deterministic identity, "
    ...
    for lease_path in receipt_paths:
        leased = {key.removeprefix("building:") for key in read(lease_path)["resources"]}
        ledger.record_many(
            snapshot, lease_path,
            [(uid, "approved-for-integration", DOC / "decision.json", installed_observation, commit)
             for uid in sorted(installed & leased)],
            effort=effort,
            request_id=BATCH + "-approved-" + snapshot + "-" + lease_path.stem,
        )
    for lease_path in receipt_paths:
        leased = {key.removeprefix("building:") for key in read(lease_path)["resources"]}
        ledger.record_many(
            snapshot, lease_path,
            [(uid, "held", DOC / "terminal-states.json", held_observations[uid], commit)
             for uid in sorted(held & leased)],
            effort=effort,
            request_id=BATCH + "-held-" + snapshot + "-" + lease_path.stem,
        )
    ...
    for lease_path in receipt_paths:
        ...
        ledger.record_many(
            snapshot, lease_path,
            [(uid, "installed-verified", DOC / "installed-acceptance.json", installed_observation, commit)
             for uid in sorted(installed & leased)],
            effort=effort,
            request_id=BATCH + "-installed-" + snapshot + "-" + lease_path.stem,
        )
    ...
    stage_name = "government-discovery-bay-terminal-v1"
