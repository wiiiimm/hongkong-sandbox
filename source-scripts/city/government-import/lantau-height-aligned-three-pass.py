...
UIDS=('landsd/190939:0','landsd/192500:0','landsd/196549:0')
def read(p):
...
def main():
 ...
 save(DOC/'assembly-proof.json',{'batch':BATCH,'policy':'bounded-current-landsd-height-alignment-v1','rows':proof,'aiCalls':0,'modelGeometryChanges':0,'placementChanges':len(rows)})
 ...
 assert validation['loaderAccepted']==validation['checksPassed']==3 and validation['exceptions']==0
 assert all(not r.get('concerns') or r['uid']=='landsd/196549:0' and r['concerns']==['sampled-terrain-above-model-bottom'] for r in validation['results'])
 assert all(not r.get('error') and r['sourcePreserved'] and r['missingTerrain']==0 and r['maxSamplerDelta']<=.004 and r['minSurfaceGap']>=-3.1 for r in metrics['rows'])
 save(STAGE/'plan.json',{'areas':[{'area':template['area'],'catalogue':rel(STAGE/'catalogue.json'),'destination':f'city/data/official-models/{BATCH}/catalogue.json'}]})
 save(STAGE/'browser-config.json',{'stage':rel(STAGE)+'/','doc':rel(DOC)+'/','catalogueURL':f'city/data/official-models/{BATCH}/catalogue.json','terrain':[],'fitBox':True,'browserUids':list(UIDS),'failureTestUids':list(UIDS)})
 save(DOC/'result.json',{'batch':BATCH,'models':3,'installedCandidates':list(UIDS),'held':[],'failures':[],'representativeUids':list(UIDS),'supportedSamplerUids':[],'missingRenderedGroundUids':[],'assemblyProofSHA256':sha(DOC/'assembly-proof.json'),'metricsSHA256':sha(DOC/'metrics.json'),'validationSHA256':sha(DOC/'validation.json'),'aiCalls':0,'geometryChanges':0,'modelGeometryChanges':0,'publication':False})
