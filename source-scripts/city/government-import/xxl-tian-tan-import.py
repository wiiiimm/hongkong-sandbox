"""Publish unchanged Tian Tan Buddha after deterministic identity, foundation and runtime acceptance."""
...
BASE = s.DOC
DOC = BASE / 'third-pass/tian-tan'
LOCAL = s.LOCAL / 'tian-tan-install'
STAGE = HERE / 'accepted/government-xxl-tian-tan-20260912'
UID = 'landsd/241332:0'
BATCH = 'government-xxl-tian-tan-20260912'
...
def start():
    claim = s.reservations.claim('codex-tian-tan-import-' + str(uuid.uuid4()), ['building:' + UID], batch=BATCH)
    assert claim['ok']
...
def owned():
    ...
    assert result['passed'] and result['policy'] == 'original-government-bounded-hillside-foundation-v1'
    ...
    decision = {
        'policy': 'original-government-bounded-hillside-foundation-v1',
        'uid': UID,
        'sourceSHA256': model['sha256'],
        'catalogueSHA256': h(STAGE / 'catalogue.json'),
        'planSHA256': h(STAGE / 'plan.json'),
        'deterministicFoundationReview': {'path': rel(result_path), 'sha256': h(result_path)},
    ...
    observation = 'Exact unchanged Tian Tan Buddha government source matched by object ID, Building CSUID and label. Exhaustive source-face checks bound five hillside-embedded faces to 0.0821% of source area; current official 5 m terrain, mobile budget, staged/live browser, picking, collision and fallback/retry checks pass. No AI review, modelling or geometry edits.'
