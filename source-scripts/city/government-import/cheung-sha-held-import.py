"""Publish the compute-only Cheung Sha / Tong Fuk / Shui Hau held-model second pass."""
...
def start():
    ...
    assert len(all_uids) == 111 and len(runtime_hold_uids) == 1
    claim = reservations.claim(
        "codex-cheung-sha-south-held-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert len(source_terminal) == 111 and held == {uid for uid, row in source_terminal.items() if BELOW_GRADE_REASON in row["reasons"]}
    ...
    decision = {
        "policy": "original-government-cheung-sha-south-held-compute-only-v1",
        ...
        "heldRows": [{
            "uid": uid,
            "reasons": source_terminal[uid]["reasons"],
            "validationError": "Upward source surfaces remain buried beyond the bounded foundation policy",
            "requiresAI": False,
            "requiresUserDecision": False,
            "needsMoreCompute": False,
            "scriptedWorkComplete": True,
            "nextDependency": "corrected-government-source-or-terrain-revision",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"The Cheung Sha / Tong Fuk / Shui Hau second compute pass installed {len(installed)} unchanged government sources after "
    ...
    held_observation = (
        "Government source has upward surfaces buried beyond the bounded foundation policy; current basic form retained. "
    ...
    terminal_result = {
        ...
        "heldReasonCounts": dict(collections.Counter(reason for uid in held for reason in source_terminal[uid]["reasons"])),
        "installedUids": sorted(installed),
    ...
    stage_name = "government-cheung-sha-south-held-terminal-v1"
