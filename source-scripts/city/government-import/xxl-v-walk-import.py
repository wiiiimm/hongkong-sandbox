"""Publish unchanged V Walk after deterministic identity, assembly, foundation and runtime acceptance."""
...
BASE = s.DOC
DOC = BASE / 'final-script-pass/v-walk'
LOCAL = s.LOCAL / 'v-walk-install'
STAGE = HERE / 'accepted/government-xxl-v-walk-20260913'
UID = 'landsd/262871:0'
BATCH = 'government-xxl-v-walk-20260913'
...
def start():
    claim = s.reservations.claim('codex-v-walk-import-' + str(uuid.uuid4()), ['building:' + uid for uid in [UID, 'landsd/178816:0', 'landsd/268497:0', 'landsd/95379:0']], batch=BATCH)
    assert claim['ok']
...
def owned():
    ...
    assert result['passed'] and result['policy'] == 'original-government-bounded-podium-assembly-v1'
    ...
    decision = {
        'policy': 'original-government-bounded-podium-assembly-v1',
        'uid': UID,
        'sourceSHA256': model['sha256'],
        'catalogueSHA256': h(STAGE / 'catalogue.json'),
        'planSHA256': h(STAGE / 'plan.json'),
        'deterministicAssemblyAndFoundationReview': {'path': rel(result_path), 'sha256': h(result_path)},
    ...
    observation = 'Exact unchanged V Walk government source matched by object ID and Building CSUID. Its bounded podium overhang intersects only the same mapped development; exhaustive source-face, mobile budget, staged/live browser, picking, collision and fallback/retry checks pass. No AI review, modelling or geometry edits.'
