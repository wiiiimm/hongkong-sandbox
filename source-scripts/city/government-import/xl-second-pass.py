"""Bounded XL-50 source/identity/terrain investigation. No AI or publication."""
...
from terrain_repair import repair_model,safe_uri as safe_terrain_uri
...
BATCH='government-xl-50-second-20260913';BASE=ROOT/'docs/astra-city/government-import/government-xl-50-20260913';DOC=BASE/'second-pass';LOCAL=HERE/'local'/BATCH
...
def prepare():
    ...
    original=read(BASE/'selection.json.gz');first=read(BASE/'results.json.gz');held={r['modelId'] for r in first['rows'] if r['humanStatus']=='held-unknown'}
    rows=[r for r in original['rows'] if r['modelId'] in held];assert len(rows)==50
    ...
    frozen={**original,'batch':BATCH,'rows':rows,'manifestSHA256':h(ROOT/'3d-viewer/city/data/manifest.json'),'previousResultSHA256':h(BASE/'results.json.gz'),'sourceCommit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'previousInstalled':0,'aiCalls':0}
...
def run():
    ...
    owned=reservations.claim('codex-xl-50-second-'+str(uuid.uuid4()),sorted(resources),batch=BATCH);assert owned['ok']
...
def terrain_triangles(path):
    path=Path(path)
    try:return context.triangles(path)
    except ValueError as original_error:
        raw=path.read_bytes()
        result=repair_model(raw,lambda uri:(path.parent/safe_terrain_uri(uri)).read_bytes())
        if result['state']!='prepared-geometry-only':raise original_error
        folder=LOCAL/'terrain-repairs'/digest(str(path.relative_to(ROOT)).encode())[:24]
        folder.mkdir(parents=True,exist_ok=True)
        for name,value in result.pop('files').items():
            target=folder/safe_terrain_uri(name);target.parent.mkdir(parents=True,exist_ok=True);target.write_bytes(value)
        proof={**result,'sourcePath':rel(path),'derivedPath':rel(folder/'geometry.gltf'),'originalError':type(original_error).__name__+': '+str(original_error),'sourceGeometryChanged':False,'aiCalls':0}
        save(folder/'proof.json',proof)
        return context.triangles(folder/'geometry.gltf')
...
def owned():
    ...
    for source in sources:
        pieces=[terrain_triangles(ROOT/p) for p in source['terrainPaths']]
    ...
    for r in rows:
        ...
        results.append(result);save(DOC/'diagnostics.json',{'rows':results,'complete':len(results)==len(rows),'aiCalls':0,'geometryChanges':0})
        if r['source']:
