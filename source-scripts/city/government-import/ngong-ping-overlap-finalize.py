"""Drop a merged Ngong Ping terrain patch that crosses an installed southwest patch."""
import json
...
DROP_DESTINATION = "city/data/government-ngong-ping-005.json"
...
def main():
    ...
    terrain["topLevelTerrainPatches"] = [
        row for row in terrain["topLevelTerrainPatches"] if row["destination"] != DROP_DESTINATION
    ]
    terrain["metricTerrainCandidates"] = [
        row for row in terrain["metricTerrainCandidates"] if row["path"] != dropped["source"]
    ]
    terrain["overlapTerrainRetained"] = [{
        "uids": uids,
        "droppedCandidate": dropped["source"],
        "installedConflict": "city/data/government-southwest-lantau-016.json",
