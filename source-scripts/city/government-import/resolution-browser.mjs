...
try{for(const width of [1280,390]){
 ...
 for(const model of browserModels){
  ...
  try{await page.waitForFunction(uid=>window.__port.stream.detailedModels.get(uid)?.active,model.uid,{timeout:60000});}catch(error){report.debug??=[];report.debug.push(await page.evaluate(uid=>{const r=window.__port,b=r.stream.getLoadedBuilding(uid),m=r.officialModels.models.get(uid);return{uid,modelKnown:!!m,buildingLoaded:!!b,buildingTile:b?.tile,selected:window.__city?.state?.selectedId,official:r.officialModels.stats,stream:r.stream.stats,cacheError:r.officialModels.cache.errors.get(uid)?.message||String(r.officialModels.cache.errors.get(uid)||''),wanted:r.officialModels.cache.wanted};},model.uid));throw error;}if(width===390)await page.evaluate(()=>{document.getElementById('building-card').hidden=true;window.__port.controlSheet?.close({restoreFocus:false});});
