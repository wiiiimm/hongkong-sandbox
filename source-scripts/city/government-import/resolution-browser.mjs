...
try{for(const width of [1280,390]){
 ...
 for(const model of catalogue.models)await page.route('**/'+assetBase+model.asset,async r=>width===390&&!allowed.has(model.uid)?r.fulfill({status:503,body:'Intentional fallback test'}):mode==='staged'?r.fulfill({body:await readFile(new URL(config.stage+model.asset,root))}):r.continue());
 ...
 for(const model of catalogue.models){
  ...
  await page.evaluate(({bounds,fitBox})=>{const r=window.__port,[a,b]=bounds,c=a.map((v,i)=>(v+b[i])/2),radius=new r.THREE.Vector3(...a).distanceTo(new r.THREE.Vector3(...b))/2,half=Math.min(r.THREE.MathUtils.degToRad(r.camera.fov/2),Math.atan(Math.tan(r.THREE.MathUtils.degToRad(r.camera.fov/2))*r.camera.aspect));let d=radius/Math.sin(half)*1.3;const direction=new r.THREE.Vector3(.5,.5,.707).normalize();
   ...
   r.closeSelection();r.controlSheet?.close({restoreFocus:false});r.camera.position.fromArray(c).addScaledVector(direction,d);r.controls.target.fromArray(c);r.controls.update();r.camera.updateMatrixWorld(true);if(fitBox){const distance=new r.THREE.Box3(new r.THREE.Vector3(...a),new r.THREE.Vector3(...b)).distanceToPoint(r.camera.position);if(distance>r.officialModels.limits.detailDistance)throw new Error('Capture camera outside initial-load detail range: '+distance);}
