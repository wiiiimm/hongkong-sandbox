"""Stage unchanged Tai O / northwest villages sources with bounded native terrain."""
...
SOURCE='government-tai-o-northwest-600-20260921';BATCH='government-tai-o-northwest-compute-20260921'
...
def main():
 ...
 regional={'city/data/terrain-tai-o.json'};installed_top=[]
 ...
 for number,group in enumerate(groups,1):
  ...
  try:
   ...
   patch_id=f'government-tai-o-northwest-{number:03d}'
   try:
   ...
   patch['id']=patch_id
   if parent['top']:
    path=STAGE/'terrain'/(patch['id']+'.json');save(path,patch);top.append({'source':rel(path),'sha256':sha(path),'destination':'city/data/'+patch['id']+'.json','resolution':patch['cell'],'area':'Tai O / northwest villages government buildings'})
 ...
 base=read(HERE.parent/'kai-tak-port/staged/catalogue.json');catalogue={k:v for k,v in base.items() if k not in ('models','counts','area')};catalogue.update(area='Tai O / northwest villages original government buildings',loadingPolicy='Lazy progressive loading after deterministic source, terrain, runtime and browser checks',counts={'packedModels':len(models)},models=models)
