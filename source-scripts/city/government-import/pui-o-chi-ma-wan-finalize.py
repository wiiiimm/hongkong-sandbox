"""Freeze the runtime-clean Pui O / Chi Ma Wan subset; no AI or geometry edits."""
import gzip,hashlib,json
from pathlib import Path

ROOT=Path(__file__).resolve().parents[3];HERE=Path(__file__).resolve().parent
SOURCE='government-pui-o-chi-ma-wan-362-20260916';BATCH='government-pui-o-chi-ma-wan-compute-20260916'
...
def main():
 validation=read(DOC/'runtime-validation.json');invalid={r['uid'] for r in validation['results'] if r['outcome']=='validation-exception'};assert len(invalid)==13
 ...
 valid={m['uid'] for m in catalogue['models']};assert len(valid)==298;catalogue['counts']={'packedModels':len(valid)};save(STAGE/'catalogue.json',catalogue);save(STAGE/'catalogue-index.json',{'models':len(valid),'catalogues':['catalogue.json']})
 ...
 for bundle_name in bundles:
  bundle=read(ROOT/bundle_name);parent=ROOT/'3d-viewer'/bundle['parentTerrainURL'];wrapper={**read(parent),'patches':read(parent).get('patches',[])+bundle['patches']};path=LOCAL/'browser-terrain'/Path(bundle['parentTerrainURL']).name;save(path,wrapper);browser_terrain.append({'source':rel(path),'destination':bundle['parentTerrainURL'],'resolution':wrapper['cell'],'area':'Pui O / Chi Ma Wan government terrain','replaces':{'url':bundle['parentTerrainURL']}})
