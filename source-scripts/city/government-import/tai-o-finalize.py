"""Freeze the runtime-clean Tai O / northwest villages subset; no AI or geometry edits."""
...
def main():
 ...
 for bundle_name in bundles:
  bundle=read(ROOT/bundle_name);parent=ROOT/'3d-viewer'/bundle['parentTerrainURL'];wrapper={**read(parent),'patches':read(parent).get('patches',[])+bundle['patches']};path=LOCAL/'browser-terrain'/Path(bundle['parentTerrainURL']).name;save(path,wrapper);browser_terrain.append({'source':rel(path),'destination':bundle['parentTerrainURL'],'resolution':wrapper['cell'],'area':'Tai O / northwest villages government terrain','replaces':{'url':bundle['parentTerrainURL']}})
