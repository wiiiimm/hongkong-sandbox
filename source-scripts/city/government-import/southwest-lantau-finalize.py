"""Freeze the runtime-clean Shek Pik / Fan Lau / southwest Lantau subset; no AI or geometry edits."""
...
def main():
 validation=read(DOC/'runtime-validation.json');invalid={r['uid'] for r in validation['results'] if r['outcome']=='validation-exception'};assert len(invalid)==0
 ...
 valid={m['uid'] for m in catalogue['models']};assert len(valid)==265;catalogue['counts']={'packedModels':len(valid)};save(STAGE/'catalogue.json',catalogue);save(STAGE/'catalogue-index.json',{'models':len(valid),'catalogues':['catalogue.json']})
 ...
 for bundle_name in bundles:
  bundle=read(ROOT/bundle_name);parent=ROOT/'3d-viewer'/bundle['parentTerrainURL'];wrapper={**read(parent),'patches':read(parent).get('patches',[])+bundle['patches']};path=LOCAL/'browser-terrain'/Path(bundle['parentTerrainURL']).name;save(path,wrapper);browser_terrain.append({'source':rel(path),'destination':bundle['parentTerrainURL'],'resolution':wrapper['cell'],'area':'Shek Pik / Fan Lau / southwest Lantau government terrain','replaces':{'url':bundle['parentTerrainURL']}})
