"""Publish the compute-only Tai O / northwest villages batch and persist terminal states."""
...
def start():
    ...
    assert len(all_uids) == 600 and len(set(all_uids)) == 600
    claim = reservations.claim(
        "codex-tai-o-northwest-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert len(all_uids) == 600 and len(installed) == installed_count and len(held) == held_count
    assert installed | held == all_uids and not (installed & held)
    assert terminal["modelsProcessed"] == 600
    assert installed_count + held_count == 600
    ...
    decision = {
        "policy": "original-government-tai-o-northwest-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"{installed_count} unchanged Tai O / northwest villages government sources installed after deterministic identity, "
    ...
    stage_name = "government-tai-o-northwest-terminal-v1"
