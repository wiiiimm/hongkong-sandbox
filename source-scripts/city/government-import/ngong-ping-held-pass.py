"""Resolve Ngong Ping / Lantau peaks holds with stronger compute-only checks."""
...
spec = importlib.util.spec_from_file_location("exact", HERE / "ngong-ping-exact-pass.py")
...
def main():
    ...
    assert len(held_rows) == 67 and RUNTIME_HOLD_UID in held_rows
    ...
    assert exact_rows[RUNTIME_HOLD_UID]["validationError"] == "Official model does not fit its matched footprint"
    seam_uids = {
    ...
    template = read(ROOT / "3d-viewer/city/data/official-models/government-ngong-ping-peaks-compute-20260918/catalogue.json")
    ...
    template.update(
        area="Ngong Ping / Lantau peaks held government sources · compute-only second pass",
