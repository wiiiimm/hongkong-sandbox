"""Compute-only pass for the two remaining Lantau XL source forms."""
import argparse
...
BATCH='government-xl-lantau-exceptions-20260923'
LIMIT=2
GROUP='xl'
...
def prepare():
    ...
    excluded=[uid for _,_,uid in entries if sections[uid] not in LANTAU_SECTIONS]
    entries=[entry for entry in entries if sections[entry[2]] in LANTAU_SECTIONS]
    if len(entries)!=LIMIT:raise ValueError(f'Expected {LIMIT} uninstalled XL sources on Lantau, found {len(entries)}')
    rows=[]
    ...
    frozen={'batch':BATCH,'nativeRun':NATIVE_RUN,'rows':rows,'manifestSHA256':h(ROOT/'3d-viewer/city/data/manifest.json'),'reviewSectionsSHA256':h(ROOT/'3d-viewer/city/data/review-sections.json'),'selectionPolicy':'all uniquely matched uninstalled XL sources on Lantau, by current review-section centroid','excludedOutsideLantauUids':excluded,'sizeGroup':GROUP,'selectionLimit':LIMIT,'sourceCommit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'aiCalls':0}
