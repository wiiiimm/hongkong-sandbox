"""Resolve Discovery Bay / Nim Shue Wan holds with stronger compute-only checks."""
...
def main():
    ...
    assert len(held_rows) == 160
    below_grade_uids = {uid for uid, row in held_rows.items() if BELOW_GRADE_REASON in row["reasons"]}
    assert len(below_grade_uids) == 7 and COLLISION_HOLD_UID in held_rows
    collision_proof = read(DOC / "collision-exception-proof.json")
    assert collision_proof["uid"] == COLLISION_HOLD_UID
    assert collision_proof["outcome"] == "validation-exception"
    assert "Source surface collision missed" in collision_proof["error"]
    runtime_hold_uids = below_grade_uids | {COLLISION_HOLD_UID}
    ...
    template.update(
        area="Discovery Bay / Nim Shue Wan held government sources · compute-only second pass",
    ...
    result = {
        ...
        "heldReasons": {uid: ([COLLISION_HOLD_REASON] if uid == COLLISION_HOLD_UID else [BELOW_GRADE_REASON]) for uid in sorted(runtime_hold_uids)},
        "failures": failures,
    ...
    save(STAGE / "browser-config.json", {
        ...
        "terrain": [],
        "fitBox": True,
        "fitBoxByModel": {"landsd/134935:0": False, "landsd/175420:0": False},
