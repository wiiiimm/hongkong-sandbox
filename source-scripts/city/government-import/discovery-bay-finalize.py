"""Freeze the runtime-clean Discovery Bay / Nim Shue Wan subset; no AI or geometry edits."""
...
def save(path,value):
 path=Path(path);path.parent.mkdir(parents=True,exist_ok=True);raw=(json.dumps(value,separators=(',',':'))+'\n').encode() if path==STAGE/'catalogue.json' else (json.dumps(value,indent=2,sort_keys=True)+'\n').encode();path.write_bytes(gzip.compress(raw,mtime=0) if path.suffix=='.gz' else raw)
...
def main():
 ...
 for bundle_name in bundles:
  bundle=read(ROOT/bundle_name);parent=ROOT/'3d-viewer'/bundle['parentTerrainURL'];wrapper={**read(parent),'patches':read(parent).get('patches',[])+bundle['patches']};path=LOCAL/'browser-terrain'/Path(bundle['parentTerrainURL']).name;save(path,wrapper);browser_terrain.append({'source':rel(path),'destination':bundle['parentTerrainURL'],'resolution':wrapper['cell'],'area':'Discovery Bay / Nim Shue Wan government terrain','replaces':{'url':bundle['parentTerrainURL']}})
 save(STAGE/'browser-config.json',{'stage':rel(STAGE)+'/','doc':rel(DOC)+'/','catalogueURL':destination,'terrain':browser_terrain,'fitBox':True,'fitBoxByModel':{'landsd/135412:0':False,'landsd/74754:0':False},'resumeBrowser':True,'browserUids':browser_uids,'failureTestUids':browser_uids})
