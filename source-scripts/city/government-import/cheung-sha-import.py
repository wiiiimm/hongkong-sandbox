"""Publish the compute-only Cheung Sha / Tong Fuk / Shui Hau batch and persist terminal states."""
...
BATCH = "government-cheung-sha-south-compute-20260921"
...
def start():
    ...
    assert len(all_uids) == 989 and len(set(all_uids)) == 989
    claim = reservations.claim(
        "codex-cheung-sha-south-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert len(all_uids) == 989 and len(installed) == installed_count and len(held) == held_count
    assert installed | held == all_uids and not (installed & held)
    assert terminal["modelsProcessed"] == 989
    assert installed_count + held_count == 989
    ...
    decision = {
        "policy": "original-government-cheung-sha-south-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"{installed_count} unchanged Cheung Sha / Tong Fuk / Shui Hau government sources installed after deterministic identity, "
    ...
    stage_name = "government-cheung-sha-south-terminal-v1"
