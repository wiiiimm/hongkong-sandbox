"""Publish the compute-only Pui O / Chi Ma Wan batch and persist terminal states."""
...
BATCH = "government-pui-o-chi-ma-wan-compute-20260916"
...
def start():
    ...
    assert len(all_uids) == 362 and len(set(all_uids)) == 362
...
def owned():
    ...
    assert len(all_uids) == 362 and len(installed) == 298 and len(held) == 64
    assert installed | held == all_uids and not (installed & held)
    assert terminal["modelsProcessed"] == 362
    assert terminal["installable"] == 298 and terminal["held"] == 64
    ...
    assert runtime["models"] == runtime["loaderAccepted"] == runtime["checksPassed"] == 298
    assert runtime["exceptions"] == 0
    assert not runtime["published"]
    metrics = read(DOC / "runtime-metrics.json")
    assert len(metrics["rows"]) == 298
    ...
    assert len(browser_uids) == 29 and browser_uids <= installed
    ...
    decision = {
        "policy": "original-government-pui-o-chi-ma-wan-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            "298 unchanged Pui O / Chi Ma Wan government sources installed after deterministic identity, "
            "full-face terrain, runtime and representative desktop/mobile browser checks. 64 terminal "
    ...
    terrain_parent = ROOT / "3d-viewer/city/data/terrain-mui-wo.json"
    before = {manifest: manifest.read_bytes(), terrain_parent: terrain_parent.read_bytes()}
    ...
    stage_name = "government-pui-o-chi-ma-wan-terminal-v1"
