...
const selected=JSON.parse(gunzipSync(selectionRaw));
let rows=selected.rows;
if(!args.selection){const previous=JSON.parse(gunzipSync(readFileSync(new URL(doc+'results.json.gz',root))));const ids=new Set(previous.rows.filter(r=>r.state==='runtime-validated-awaiting-acceptance').map(r=>r.uid));rows=rows.filter(r=>ids.has(r.uid));assert.equal(rows.length,61);}
assert(rows.length>0&&rows.length<=1000);assert.equal(new Set(rows.map(r=>r.uid)).size,rows.length);
const output=new URL(args.out||doc+'acceptance/metrics.json',root);mkdirSync(new URL('.',output),{recursive:true});
...
for(const r of rows){let model;
 try{const e=r.candidate.entry,raw=readFileSync(new URL(`${args.candidates||`source-scripts/city/government-import/local/${batch}/candidates`}/${e.asset}`,root));assert.equal(hash(raw),e.sha256);const source=load('3d-viewer/'+r.source.tile).buildings.find(b=>b.uid===r.uid);assert.deepEqual(source,r.source.building);
  ...
  const bottom=e.worldBounds[0][1];let count=0,lowCount=0,minGap=Infinity,maxLowGap=-Infinity,minLowGap=Infinity,maxSamplerDelta=0,missing=0,samplerDisagreementCount=0,minSamplerDisagreementDelta=Infinity,minSamplerDisagreementDrawnClearance=Infinity,minSamplerDisagreementRuntimeClearance=Infinity,minMissingRuntimeClearance=Infinity;
  const check=(x,y,z)=>{const drawn=surface.height(x,z);count++;if(drawn===null){missing++;minMissingRuntimeClearance=Math.min(minMissingRuntimeClearance,y-sampler.height(x,z));return;}const sampled=sampler.height(x,z),gap=y-drawn,delta=Math.abs(sampled-drawn);minGap=Math.min(minGap,gap);maxSamplerDelta=Math.max(maxSamplerDelta,delta);if(delta>.004){samplerDisagreementCount++;minSamplerDisagreementDelta=Math.min(minSamplerDisagreementDelta,delta);minSamplerDisagreementDrawnClearance=Math.min(minSamplerDisagreementDrawnClearance,y-drawn);minSamplerDisagreementRuntimeClearance=Math.min(minSamplerDisagreementRuntimeClearance,y-sampled);}if(y<=bottom+.35){lowCount++;minLowGap=Math.min(minLowGap,gap);maxLowGap=Math.max(maxLowGap,gap);}};
...
if(args['geometry-out']){const p=new URL(args['geometry-out'],root);mkdirSync(new URL('.',p),{recursive:true});writeFileSync(p,gzipSync(JSON.stringify({inputHashes:inputs,rows:geometry})));}
