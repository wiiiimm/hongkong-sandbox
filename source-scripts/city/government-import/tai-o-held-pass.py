"""Resolve Tai O / northwest villages holds with stronger compute-only checks."""
...
spec = importlib.util.spec_from_file_location("exact", HERE / "tai-o-exact-pass.py")
...
def main():
    ...
    assert len(held_rows) == 78 and RUNTIME_HOLD_UID in held_rows
    ...
    install_uids = set(held_rows) - {RUNTIME_HOLD_UID}
    assert set(geometries) >= install_uids
    assert "below-grade-source-surfaces-exceed-bounded-foundation-policy" in held_rows[RUNTIME_HOLD_UID]["reasons"]
    seam_uids = {
    ...
    template = read(ROOT / "3d-viewer/city/data/official-models/government-tai-o-northwest-compute-20260921/catalogue.json")
    ...
    template.update(
        area="Tai O / northwest villages held government sources · compute-only second pass",
