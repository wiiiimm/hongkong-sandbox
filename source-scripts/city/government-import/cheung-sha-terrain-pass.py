"""Stage unchanged Cheung Sha / Tong Fuk / Shui Hau sources with bounded native terrain."""
...
SOURCE='government-cheung-sha-south-989-20260921';BATCH='government-cheung-sha-south-compute-20260921'
...
def main():
 ...
 for number,group in enumerate(groups,1):
  ...
  try:
   ...
   patch_id=f'government-cheung-sha-south-{number:03d}'
   try:
   ...
   patch['id']=patch_id
   if parent['top']:
    path=STAGE/'terrain'/(patch['id']+'.json');save(path,patch);top.append({'source':rel(path),'sha256':sha(path),'destination':'city/data/'+patch['id']+'.json','resolution':patch['cell'],'area':'Cheung Sha / Tong Fuk / Shui Hau government buildings'})
 ...
 base=read(HERE.parent/'kai-tak-port/staged/catalogue.json');catalogue={k:v for k,v in base.items() if k not in ('models','counts','area')};catalogue.update(area='Cheung Sha / Tong Fuk / Shui Hau original government buildings',loadingPolicy='Lazy progressive loading after deterministic source, terrain, runtime and browser checks',counts={'packedModels':len(models)},models=models)
