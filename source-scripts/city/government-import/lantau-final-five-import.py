...
def start():
    ...
    assert len(all_uids) == 5 and len(set(all_uids)) == 5
...
def owned():
    ...
    assert len(source_rows) == result["models"] == 5
    held = set()
    ...
    decision = {
        "policy": "original-government-lantau-final-five-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"The final Lantau compute pass installed {len(installed)} unchanged government sources after "
    ...
    stage_name = "government-lantau-final-five-terminal-v1"
