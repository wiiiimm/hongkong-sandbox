"""Publish the compute-only Discovery Bay / Nim Shue Wan held-model second pass."""
...
ROOT = HERE.parents[2]
SOURCE_BATCH = "government-discovery-bay-1103-20260921"
BATCH = "government-discovery-bay-held-160-20260921"
SOURCE_DOC = ROOT / "docs/astra-city/government-import" / SOURCE_BATCH
DOC = SOURCE_DOC / "held-160"
STAGE = HERE / "accepted" / BATCH
LOCAL = HERE / "local" / BATCH
BELOW_GRADE_REASON = "below-grade-source-surfaces-exceed-bounded-foundation-policy"
COLLISION_HOLD_UID = "landsd/288285:0"
COLLISION_HOLD_REASON = "source-surface-collision-missed"
...
def start():
    ...
    runtime_hold_uids = {row["uid"] for row in terminal["heldRows"] if BELOW_GRADE_REASON in row["reasons"]}
    assert len(all_uids) == 160 and len(runtime_hold_uids) == 7
...
def owned():
    ...
    source_terminal = {row["uid"]: row for row in read(SOURCE_DOC / "terminal-states.json")["heldRows"]}
    held = set(result["held"])
    below_grade = {uid for uid, row in source_terminal.items() if BELOW_GRADE_REASON in row["reasons"]}
    assert len(source_terminal) == 160 and len(below_grade) == 7
    assert held == below_grade | {COLLISION_HOLD_UID}
    assert result["heldReasons"] == {uid: ([COLLISION_HOLD_REASON] if uid == COLLISION_HOLD_UID else [BELOW_GRADE_REASON]) for uid in sorted(held)}
    proof = read(DOC / "collision-exception-proof.json")
    assert proof["uid"] == COLLISION_HOLD_UID and "Source surface collision missed" in proof["error"]
    assert result["models"] == len(source_terminal) - len(held)
    ...
    assert all_uids == set(source_terminal) and not (installed & held)
    ...
    source_selection = read(SOURCE_DOC / "check-selection.json.gz")
    source_rows = {row["uid"]: row for row in source_selection["rows"]}
    assert all_uids <= set(source_rows) and set(source_terminal) >= all_uids
    evidence_paths = {
    ...
    decision = {
        "policy": "original-government-discovery-bay-held-compute-only-v1",
        ...
        "heldRows": [{
            "uid": uid,
            "reasons": sorted(set(source_terminal[uid]["reasons"]) | set(result["heldReasons"][uid])),
            "validationError": (
                proof["error"] if uid == COLLISION_HOLD_UID
                else "Upward source surfaces remain buried beyond the bounded foundation policy"
            ),
            "requiresAI": False,
            ...
            "nextDependency": (
                "source-collision-runtime-investigation" if uid == COLLISION_HOLD_UID
                else "corrected-government-source-or-terrain-revision"
            ),
    ...
    for uid in sorted(all_uids):
        ...
        parts[uid] = {
            ...
            "classification": (
                ...
                if uid in installed else "script-blocked-original-government-import"
            ),
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            f"The Discovery Bay / Nim Shue Wan second compute pass installed {len(installed)} unchanged government sources after "
            "stitched-terrain, retained-support, runtime and representative browser checks. One below-grade government source "
            "match and one collision-check exception remain held. No AI review or geometry edits."
        ),
    })
    ...
    installed_observation = (
        "Exact unchanged government source passed deterministic stitched-terrain, retained-basic-support, runtime and "
        "representative desktop/mobile browser checks. No AI review, remodelling, simplification or geometry edit."
    )
    held_observation = (
        "Unchanged government source held after below-grade or collision runtime evidence; current basic form retained. "
        "No AI review, geometry edit or user decision made."
    )
    ledger.record_many(
        snapshot,
        receipt_path,
    ...
    ledger.record_many(
        snapshot,
        receipt_path,
        [(uid, "held", DOC / "decision.json", held_observation, commit) for uid in sorted(held)],
        effort=effort,
    ...
    stage_name = "government-discovery-bay-held-terminal-v1"
