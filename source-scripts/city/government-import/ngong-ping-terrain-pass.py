"""Stage unchanged Ngong Ping / Lantau peaks sources with bounded native terrain."""
...
def main():
 ...
 regional={'city/data/terrain-ngong-ping.json','city/data/government-southwest-lantau-016.json','city/data/government-southwest-lantau-017.json'};installed_top=[]
 ...
 for number,group in enumerate(groups,1):
  ...
  try:
   ...
   patch_id=f'government-ngong-ping-{number:03d}'
   try:
   ...
   patch['id']=patch_id
   if parent['top']:
    path=STAGE/'terrain'/(patch['id']+'.json');save(path,patch);top.append({'source':rel(path),'sha256':sha(path),'destination':'city/data/'+patch['id']+'.json','resolution':patch['cell'],'area':'Ngong Ping / Lantau peaks government buildings'})
 ...
 base=read(HERE.parent/'kai-tak-port/staged/catalogue.json');catalogue={k:v for k,v in base.items() if k not in ('models','counts','area')};catalogue.update(area='Ngong Ping / Lantau peaks original government buildings',loadingPolicy='Lazy progressive loading after deterministic source, terrain, runtime and browser checks',counts={'packedModels':len(models)},models=models)
