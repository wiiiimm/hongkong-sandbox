...
def owned():
 ...
 for uid,support_uid in PAIRS.items():
  result=final[uid];identity,foundation,run=result['identity'],result['foundation'],result['runtime'];assert result['publicationCandidate'] and result['scriptedWorkComplete'] and result['aiCalls']==result['modelGeometryChanges']==0;assert identity['exactObjectAndCSUID'] and identity['targetCoveredBySourceProjection']>.98 and identity['sourceExcessFraction']<.08 and identity['unrelatedIntersectingForms']==0;assert foundation['fullyBuriedTriangles']==0;assert run['sourcePreserved'] and run['mobileBudgetPassed'] and not run['missingTerrain']
 ...
 decision={'uids':list(PAIRS),'policy':'original-government-supported-xl-towers-v1','passed':not reasons,'reasons':sorted(set(reasons)),'supportProofSHA256':h(DOC/'support-proof.json'),'aiCalls':0,'modelGeometryChanges':0,'publication':False};save(DOC/'result.json',decision)
 if decision['passed']:
