"""Publish the compute-only Ngong Ping / Lantau peaks held-model second pass."""
...
BATCH = "government-ngong-ping-peaks-held-67-20260918"
SOURCE_DOC = ROOT / "docs/astra-city/government-import" / SOURCE_BATCH
DOC = SOURCE_DOC / "held-67"
STAGE = HERE / "accepted" / BATCH
LOCAL = HERE / "local" / BATCH
RUNTIME_HOLD_UID = "landsd/168823:0"
...
def start():
    ...
    assert len(all_uids) == 67 and RUNTIME_HOLD_UID in all_uids
    claim = reservations.claim(
        "codex-ngong-ping-peaks-held-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert result["models"] == 66 and result["held"] == [RUNTIME_HOLD_UID]
    ...
    assert len(installed) == 66 and len(all_uids) == 67 and not (installed & held)
    ...
    assert len(metrics["rows"]) == 66 and metrics["aiCalls"] == metrics["geometryChanges"] == 0
    ...
    assert validation["models"] == validation["loaderAccepted"] == validation["checksPassed"] == 66
    ...
    decision = {
        "policy": "original-government-ngong-ping-held-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            "The Ngong Ping / Lantau peaks second compute pass installed 66 unchanged government sources after "
    ...
    terminal_result = {
        ...
        "heldReasonCounts": {"runtime-validation-exception": 1},
        "installedUids": sorted(installed),
    ...
    stage_name = "government-ngong-ping-peaks-held-terminal-v1"
