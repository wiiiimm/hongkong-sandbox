"""Publish the compute-only Shek Pik / Fan Lau / southwest Lantau batch and persist terminal states."""
...
BATCH = "government-shek-pik-fan-lau-compute-20260917"
...
def start():
    ...
    assert len(all_uids) == 304 and len(set(all_uids)) == 304
    claim = reservations.claim(
        "codex-shek-pik-fan-lau-" + str(uuid.uuid4()),
...
def owned():
    ...
    assert len(all_uids) == 304 and len(installed) == 265 and len(held) == 39
    assert installed | held == all_uids and not (installed & held)
    assert terminal["modelsProcessed"] == 304
    assert terminal["installable"] == 265 and terminal["held"] == 39
    ...
    assert runtime["models"] == runtime["loaderAccepted"] == runtime["checksPassed"] == 265
    assert runtime["exceptions"] == 0
    assert not runtime["published"]
    metrics = read(DOC / "runtime-metrics.json")
    assert len(metrics["rows"]) == 265
    ...
    assert len(browser_uids) == 35 and browser_uids <= installed
    ...
    decision = {
        "policy": "original-government-southwest-lantau-compute-only-v1",
    ...
    save(inventory_path, {
        **inventory,
        ...
        "parts": ordered,
        "qualification": (
            "265 unchanged Shek Pik / Fan Lau / southwest Lantau government sources installed after deterministic identity, "
            "full-face terrain, runtime and representative desktop/mobile browser checks. 39 terminal "
    ...
    stage_name = "government-southwest-lantau-terminal-v1"
