"""Exact source-terrain pass for the Shek Pik / Fan Lau / southwest Lantau government batch.
...
ROOT = HERE.parents[2]
BATCH = "government-shek-pik-fan-lau-304-20260917"
...
def exact():
    ...
    assert set(selection) == set(geometries) and len(selection) == 304
    ...
    results = []
    for number, uid in enumerate(sorted(selection), 1):
        row = selection[uid]
    ...
    report = {
        ...
        "qualification": (
            "All 304 sources received exact identifier, projected assembly, and full-face "
            "same-sheet source-TIN checks. Source geometry remains unchanged."
        ),
    }
