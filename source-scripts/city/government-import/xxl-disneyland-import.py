"""Publish unchanged Hong Kong Disneyland after bounded roof and edge-overhang acceptance."""
...
BASE = s.DOC
DOC = BASE / 'fourth-pass/disneyland'
LOCAL = s.LOCAL / 'disneyland-install'
STAGE = HERE / 'accepted/government-xxl-disneyland-20260913'
UID = 'landsd/108268:0'
BATCH = 'government-xxl-disneyland-20260913'
...
def start():
    claim = s.reservations.claim('codex-disneyland-import-' + str(uuid.uuid4()), ['building:' + UID], batch=BATCH)
    assert claim['ok']
...
def owned():
    ...
    assert result['passed'] and result['policy'] == 'original-government-bounded-roof-overhang-v1'
    ...
    decision = {
        'policy': 'original-government-bounded-roof-overhang-v1',
        'uid': UID,
        'sourceSHA256': model['sha256'],
        'catalogueSHA256': h(STAGE / 'catalogue.json'),
        'planSHA256': h(STAGE / 'plan.json'),
        'deterministicRoofOverhangReview': {'path': rel(result_path), 'sha256': h(result_path)},
    ...
    observation = 'Exact unchanged Hong Kong Disneyland government source matched by object ID and Building CSUID. Its projection covers 97.4% of the recorded form; the 8.6% roof and edge overhang stays within 7.2 m and overlaps one unnamed OSM outline by only 1.43 m2. Source, terrain, mobile budget, staged/live browser, picking, collision and fallback/retry checks pass. No AI review, modelling or geometry edits.'
