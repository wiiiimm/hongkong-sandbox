"""Publish unchanged Stonecutters sedimentation tanks after bounded exterior-foundation acceptance."""
...
BASE = s.DOC
DOC = BASE / 'fourth-pass/stonecutters'
LOCAL = s.LOCAL / 'stonecutters-install'
STAGE = HERE / 'accepted/government-xxl-stonecutters-20260913'
UID = 'landsd/128464:0'
BATCH = 'government-xxl-stonecutters-20260913'
...
def start():
    claim = s.reservations.claim('codex-stonecutters-import-' + str(uuid.uuid4()), ['building:' + UID], batch=BATCH)
    assert claim['ok']
...
def owned():
    ...
    assert result['passed'] and result['policy'] == 'original-government-bounded-exterior-foundation-v1'
    ...
    decision = {
        'policy': 'original-government-bounded-exterior-foundation-v1',
        'uid': UID,
        'sourceSHA256': model['sha256'],
        'catalogueSHA256': h(STAGE / 'catalogue.json'),
        'planSHA256': h(STAGE / 'plan.json'),
        'deterministicExteriorFoundationReview': {'path': rel(result_path), 'sha256': h(result_path)},
    ...
    observation = 'Exact unchanged Stonecutters sedimentation-tank government source matched by object ID and Building CSUID. Fifteen exterior edge faces totaling 2.80 m2 are bounded against original terrain; source, mobile budget, staged/live browser, picking, collision and fallback/retry checks pass. No AI review, modelling or geometry edits.'
