...
try{
 ...
 await old.goto('http://127.0.0.1:4176/whampoa-comparison.html');
 ...
 await page.goto('http://127.0.0.1:4176/whampoa-comparison.html');await page.waitForFunction(()=>window.__trial?.ready);
 ...
 await fail.goto('http://127.0.0.1:4176/whampoa-comparison.html');await fail.waitForFunction(()=>window.__trial?.ready);
 ...
 const fatal=await browser.newPage();await fatal.route('**/manifest.json',r=>r.fulfill({status:503,body:'Unavailable'}));await fatal.goto('http://127.0.0.1:4176/whampoa-comparison.html');await fatal.waitForFunction(()=>document.querySelector('#load-status')?.getAttribute('role')==='alert');assert.match(await fatal.locator('#load-message').innerText(),/HTTP 503/);report.manifestFailureVisible=true;await fatal.close();
