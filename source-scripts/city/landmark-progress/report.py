...
def build(root=ROOT):
    ...
    for landmark in landmarks:
        ...
        staged = {u for u,p in parts.items() if p.get('stagedAsset')}
        ...
        result.append(dict(id=landmark['id'], name=landmark['name'], preparationState=row['state'],
                           ...
                           'Resolve identity/component membership' if row['state'] in ('no-identity','ambiguous') else
    ...
    return dict(schemaVersion=1, landmarks=len(result), readyForReview=counts.get('ready-for-review',0),
                ...
                inputHashes={str(p.relative_to(root)):digest(p) for p in (registry_path,batch_path,reviews_path)},
                rows=result)
