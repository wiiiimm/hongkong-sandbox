...
const path='source-scripts/city/whampoa-special/candidates/',catalogue=await read(path+'catalogue.json'),models=new Map(catalogue.models.map(m=>[m.uid,m]));
const groups=[{id:'whampoa-ship',uids:['landsd/144948:0','landsd/225581:0']}];
...
const terrainBundle=await read('docs/astra-city/whampoa-special/terrain-patches.json');
...
try{
 for(const mobile of process.env.IDENTITY_FOUNDATIONS_ONLY? [false]:[false,true]){
  ...
  let inject=false;const url='city/data/official-models/whampoa-special/catalogue.json';
  ...
  await page.route('**/city/data/manifest.json',async r=>{const response=await r.fetch(),data=await response.json();if(inject){data.officialModelCatalogues=[...data.officialModelCatalogues,url];for(const bundle of terrainBundle.bundles)if(bundle.parentTerrainURL==='city/data/terrain.json')for(const p of bundle.patches)data.terrainPatches.push({url:'city/data/whampoa-review-terrain.json',resolution:1});}await r.fulfill({response,json:data});});
  for(const bundle of terrainBundle.bundles)for(const patch of bundle.patches){const data=await read(patch.path);if(bundle.parentTerrainURL==='city/data/terrain.json')await page.route('**/city/data/whampoa-review-terrain.json',r=>r.fulfill({json:data}));else await page.route('**/'+bundle.parentTerrainURL,async r=>{const response=await r.fetch(),parent=await response.json();if(inject)(parent.patches||=[]).push(data);await r.fulfill({response,json:parent});});}
  for(const phase of mobile||process.env.IDENTITY_FOUNDATIONS_ONLY?['after']:['before','after']){
   ...
   for(const g of groups){
    ...
    for(const direction of process.env.IDENTITY_FOUNDATIONS_ONLY?['foundation']:phase==='before'||mobile||g.id==='langham-ancillary'?['southwest']:['southwest','northeast']){
     ...
     if(process.env.WHAMPOA_ISOLATED&&inject)await page.evaluate(uids=>window.__review.scene.traverse(o=>{if(o.isMesh)o.visible=uids.includes(o.userData.officialBuildingUid);}),g.uids);
...
}finally{report.finished=new Date().toISOString();await writeFile(new URL(process.env.IDENTITY_FOUNDATIONS_ONLY?'foundations.json':'report.json',output),JSON.stringify(report,null,2)+'\n');await browser.close();}
