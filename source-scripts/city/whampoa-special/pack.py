...
for p in ['/tmp/astra-whampoa-lease.json']:assert reservations.owns(json.loads(pathlib.Path(p).read_text()))
...
c.close();catalogue=read(ROOT/'source-scripts/city/grounded-model-review/candidates/catalogue.json');catalogue.update(area='The Whampoa ship and platform special request',models=[r['record']for r in results],counts={'packedModels':len(results)})
for model in catalogue['models']:model['proceduralWindows']=False
(out/'catalogue.json').write_text(json.dumps(catalogue,indent=2)+'\n');(DOC/'packing.json').write_text(json.dumps({'issue':'HKS-219','results':results,'published':False,'sourceBytesPreserved':True},indent=2)+'\n')
