...
ROOT=pathlib.Path(__file__).resolve().parents[3];HERE=ROOT/'source-scripts/city/landmark-acquisition/batches/grand-promenade-neighbour-20260909';DOC=ROOT/'docs/astra-city/residual-support-review/grand'
sys.path.insert(0,str(ROOT/'source-scripts/city/shared-modelling'));import reservations
for p in ['/tmp/astra-grand-neighbours-lease.json']:assert reservations.owns(json.loads(pathlib.Path(p).read_text()))
...
c.close();catalogue=read(ROOT/'source-scripts/city/grounded-model-review/candidates/catalogue.json');catalogue.update(area='Residual exact native support review',models=[r['record']for r in results],counts={'packedModels':len(results)})
(out/'catalogue.json').write_text(json.dumps(catalogue,indent=2)+'\n');(DOC/'neighbour-packing.json').write_text(json.dumps({'issue':'HKS-214','results':results,'published':False,'sourceBytesPreserved':True},indent=2)+'\n')
