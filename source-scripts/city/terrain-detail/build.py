...
def build_patch(dem_source=DEFAULT,bbox=None,output=None,detail_paths=None,evidence=None,area='Mui Wo',rendered_transition=False):
 output=output or OUT/'terrain-mui-wo.json'
 evidence=evidence or ROOT/'docs/astra-city/mui-wo-buildings/extension'
 coarse=json.loads((OUT/'terrain.json').read_text());g=coarse['meta']['georef'];cw=coarse['w']
 bbox=bbox or json.loads((OUT/'mui-wo-buildings.json').read_text())['bboxWGS84'];project=Transformer.from_crs(4326,2326,always_xy=True)
 ...
 with zipfile.ZipFile(dem_source) as z:
 ...
 if len(detail_paths)>1:
  (evidence/'terrain-mosaic.json').write_text(json.dumps(mosaic,indent=2)+'\n')
 patch={'w':w,'h':h,'cell':5,'elev':np.round(fine.astype(np.float64),2).flatten().tolist(),'vegetation':vegetation,'coarseCells':[c0,r0,c1,r1],'meta':{'georef':{'aE':5,'bE':E,'aN':-5,'bN':N,'W':w,'H':h},'source':source,'title':area+' · Lands Department 5 m DTM','detailSources':detail_sources}}
 if rendered_transition:
  # Preserve raw DTM/TIN samples and masks. Interpolate the coarse *rendered*
  # vertices at the border so water clamping cannot open a coastal seam.
  rendered=[None]*(w*h)
  for r in range(h):
   for c in range(w):
    alpha=min(1,min(c,r,w-1-c,h-1-r)/14)
    if alpha==1:continue
    col=c0+c/14;row=r0+r/14;i=int(col);j=int(row);u=col-i;v=row-j
    a,b,d,e=[max(1.2,coarse['elev'][q]) if coarse['elev'][q]>0 else -4 for q in (j*cw+i,j*cw+i+1,(j+1)*cw+i,(j+1)*cw+i+1)]
    old=a+(b-a)*u+(d-a)*v if u+v<=1 else e+(d-e)*(1-u)+(b-e)*(1-v)
    value=patch['elev'][r*w+c];current=max(1.2,value) if value>0 else -4
    adjusted=current*alpha+old*(1-alpha)
    if abs(adjusted-current)>1e-8:rendered[r*w+c]=round(adjusted,6)
  patch['renderedElev']=rendered
  patch['meta']['renderedTransition']={'widthMetres':70,'vertices':sum(x is not None for x in rendered),'policy':'Only outer transition display heights interpolate the existing coarse rendered triangles. Raw elevations and their water mask remain unchanged; interior source TIN and buildings are unshifted.'}
 ...
 return patch,mosaic
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--source',type=pathlib.Path,default=DEFAULT);args=ap.parse_args();build_patch(dem_source=args.source)
