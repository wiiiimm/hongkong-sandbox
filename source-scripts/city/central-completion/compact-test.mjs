...
const batch=process.argv[2]||'compact';
assert(['compact','compact-followup'].includes(batch));
...
const evidence=JSON.parse(await readFile(new URL(`${batch}-assets.json`,doc),'utf8'));
const selection=JSON.parse(gunzipSync(await readFile(new URL('building-selection.json.gz',here))));
const selected=new Map(selection.buildings.map(b=>[b.uid,b]));
const hash=bytes=>createHash('sha256').update(bytes).digest('hex');
let totalTriangles=0;const results=[];
const allCatalogue=batch==='compact'?JSON.parse(await readFile(new URL('compact/catalogue-all.json',here),'utf8')):catalogue;
