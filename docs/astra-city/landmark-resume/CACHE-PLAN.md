...
7. **Device runtime setup remains necessary.** The stage adapter defaults to one user's Node executable (overridable with `--node`). Gallery browser selection is now portable: `CHROME_PATH` or `CHROMIUM_EXECUTABLE_PATH` overrides are checked explicitly, then local macOS/Playwright/Linux/Windows browser locations are considered. An invalid explicit path fails instead of silently falling back. The preview URL remains `127.0.0.1:4176`, so start the matching local preview. Python needs the existing geometry dependencies, including NumPy, Shapely and pyproj.
...
## Optional completed review evidence

The completed gallery checkpoint (`7ef06eaa`) contains **166 captured assemblies and 332 full-resolution JPEGs**, covering the same 273-candidate preflight snapshot. Preserve the **54,625,709 bytes (54.63 MB) of ignored JPEGs** for later modelling review if desired. The tracked `contact.html` is a 3.79 MB compact preview; it is not a substitute for the full-resolution views.

The inventory's `optional-review-evidence` profile includes `docs/astra-city/landmark-preflight/gallery/3887f2f23fbad306/` images and per-worker reports/indexes, plus the tracked merged `gallery-summary.json`, `gallery-images.json` and `contact.html`. The merged summary is authoritative; older per-worker or local directory reports are retained as provenance and must not be mistaken for final aggregate counts. This profile contains 347 files: **56.68 MB outside Git**, including the JPEGs and their ignored metadata, plus **5.50 MB already tracked** (62.18 MB total). It is excluded from both core working-closure and unmodified-CLI totals. Its ignored metadata should travel with the JPEGs, while tracked evidence comes from the matching Git checkout.

Capture completion is not visual acceptance: the summary still records four unresolved camera views and ten candidate parts never active in the captured views. Preserve those limitations with the images. Reported image SHA-256 values are recorded as existing evidence only; this collector does not independently verify image bytes or create a backup/upload manifest.

This plan does not authorise modelling, geometry changes, terrain application or publication.
