...
Reserve the GPU before running. `CITY_URL` and `CHROME_PATH` are optional overrides. The main report is `browser/verification.json`, with direct screenshots at 390×844, 320×844, 390×480 and 1440×1000 (device scale 1, Chrome 152). It verifies the collapsed and expanded states; all four tabs; full clock and lower controls; search; Explore/Walk/Fly/Stargaze; layer-independent meteor and tide input; native keyboard operations; handle gestures; and listener disposal. No page exceptions or WebGL/shader errors occurred.
...
Exported phone, desktop, clock, lower-weather and short-viewport screenshots were inspected visually: typography and control labels remain legible, the full dial fits at the top of the Time tab, and scrolled controls stop above the persistent dock. The first failed CSS-cascade check is preserved as `browser/first-pass.json` and `first-pass-overlap.png`; its cause was the old mobile `.explorer.open {display:block}` rule overriding flex layout, and the final rule corrects it.
...
The subsequent HKS-189 visibility correction groups the dial, toggle and speed slider at the top of Time, ahead of the calendar controls. [Current first-open screenshots and multiplier checks](../time-cycle/README.md) supersede the earlier scroll-to-reach speed-control screenshots.

## Separate Time tab · 7 September 2026

The global clock now has its own tab: **Places · Time · Sky · Weather**. Time contains the 24-hour dial, timelapse toggle and 1×–7,200× speed slider, presets, exact time, live/manual clock, calendar and distant-light shimmer. Sky contains astronomy readouts, constellations, meteors and viewing bearings. Entering Stargaze still selects Sky automatically. The existing control IDs and environment handlers are preserved; the generic sheet controller needs no changes.

The refreshed control-sheet browser run passes 12 groups, including ownership of each control, four-tab keyboard navigation, state persistence, native input isolation and mobile layout at 320/390 px. The timelapse run passes all 10 groups, including date rollover, multiplier speed, shadows, lights, pause/resume and independent live weather/tide clocks. First-open speed controls fit without scrolling at 320/390/760/1024/1440 px. New exported Time screenshots at 390 and 1440 px were inspected. Existing browser scripts now select Time before clock controls; their unchanged subsystem scenarios were not all rerun.
