...
The subsequent [online-reference and live integration review](ONLINE-REFERENCE-REVIEW.md) records the requested Google Maps/Open3Dhk comparison and confirms all 275 model records loading through the real city tile pipeline. Its screenshots and `live-model-check.json` are separate from the temporary original-model fixture. `compare_model_outline_heights.py` also preserves differences between the source model extrema and dated outline height fields; the application correctly labels its numeric height as outline height.
