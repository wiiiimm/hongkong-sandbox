...
try{
 for(const provider of ['google','open3dhk']){
  const page=await browser.newPage({viewport:{width:1600,height:1000},deviceScaleFactor:1});
  const errors=[];page.on('pageerror',e=>errors.push(e.message));
  for(const target of selected){
   const url=provider==='google'?`https://www.google.com/maps/@${target.lat},${target.lon},18z/data=!3m1!1e3`:`https://3d.map.gov.hk/mapviewer/app/map?flyto=${encodeURIComponent([target.lat,target.lon,650,0,60,0].join(','))}&l=en-US`;
   let navigationError=null;try{await page.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await page.waitForTimeout(12000);}catch(e){navigationError=e.message;}
 ...
 await writeFile(path.join(here,process.env.REF_ALL==='1'?'online-reference-check.json':'online-reference-access.json'),JSON.stringify({checkedAt:new Date().toISOString(),method:'Normal headless Chrome navigation and viewport captures only. No downloaded imagery tiles, account access, or access-block workarounds.',results},null,2)+'\n');
