...
Agent: `whampoa_ship_model` / GPT-6 Astra. Separate addition to the original 213-landmark baseline. Two unchanged Lands Department non-textured native model components, 4,808 triangles, 63,584 compressed runtime bytes. Both have now passed root's installed feature-branch verification in `../model-integration-20260909/whampoa-capitol-live/after/verification.json`; this is not production deployment proof.
...
`browser-terrain/` contains six normal-scene before/after/day/night/desktop/emulated-mobile captures. `browser-isolated/` contains six labelled source-inspection captures; adjacent apartment blocks occlude parts of the ship in the normal wide mobile view. Isolation is diagnostic, not a claim that surrounding buildings disappear in the product. Both target IDs load and pick correctly. Root's installed verification now passes native picking/collision, terrain-render agreement, mobile night, local walk/fly and failed-load fallback/Retry. `close-view/` adds normal-scene live aerial views with the whole ship visible, no data interception, no hidden geometry and final visibility checks after streaming settles.
...
Neon request job `8684beb4926859367e8e732be2dab9f4fcf8497b383f421eb40c27a1dab43932`; source inventory supplement `41bc2bb6beaf282f` is merged into shared snapshot `f61930a5e4dfa572`. Both source reviews are `installed-verified`. The native-geometry job is complete; `neon-job-result.json` records the installed evidence hash. A separate appearance-review job remains pending, with source-backed scope in `appearance-follow-on.json`: daylight paint/glazing confirmation, semantic material masks, bow markings and mast lighting. No exact paint RGB or invented photo textures are claimed.
