...
| --- | --- | --- |
| HKS-167 · Mui Wo official 3D extension | Heisenberg (`complete_geography`) | Existing model/terrain conversion, stable government IDs and current territory publication |
| HKS-168 · Original shooting stars | Kant (`sky_stargazing`) | Shared original meteor pool; primary agent owns Sky controls |
| HKS-169 · Lightning, thunder and weather audio | Curie (`weather`) | Original synthesiser/storm maths; primary agent owns Weather controls |

Primary agent owns city `environment.js`, `app.js`, `city.html` and CSS for the combined control changes, plus integration, final checks and Linear updates. Geography exclusively owns live terrain/model publication during this batch. GPU browser runs are serialised for reliable measurements. Reuse existing source data, assets and code first; only fill missing adapters/behaviours. Move implemented, verified work to In Review; do not label the whole region or original-feature roll-up complete before its acceptance passes.
