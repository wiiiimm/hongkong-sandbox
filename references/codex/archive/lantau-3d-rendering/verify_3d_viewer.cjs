...
const VIEWER_URL = 'http://127.0.0.1:4173/lantau-3d-rendering/lantau-3d-viewer.html';
...
(async () => {
  ...
  await verifyViewport(page, { width: 1440, height: 960 }, 'codex/lantau-3d-rendering/lantau-3d-viewer-desktop.png');
  await verifyViewport(page, { width: 390, height: 844 }, 'codex/lantau-3d-rendering/lantau-3d-viewer-mobile.png');
  await browser.close();
})().catch(error => {
  console.error(error);
  process.exit(1);
