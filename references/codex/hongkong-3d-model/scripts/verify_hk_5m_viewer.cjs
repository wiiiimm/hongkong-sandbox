...
const VIEWER_URL = 'http://127.0.0.1:4173/lantau-3d-rendering-hk-5m/output/lantau-hk-5m-3d-viewer.html';
...
async function verifyViewport(page, viewport, output) {
  ...
  if (!readout || !readout.includes('HK 5 m DTM')) {
    throw new Error(`Readout did not load HK 5 m mesh metadata: ${readout}`);
  }
...
(async () => {
  ...
  await verifyViewport(page, { width: 1440, height: 960 }, 'codex/lantau-3d-rendering-hk-5m/output/lantau-hk-5m-3d-viewer-desktop.png');
  await verifyViewport(page, { width: 390, height: 844 }, 'codex/lantau-3d-rendering-hk-5m/output/lantau-hk-5m-3d-viewer-mobile.png');
  await browser.close();
})().catch(error => {
  console.error(error);
  process.exit(1);
